...
// This is an abstract base class implementing a single function
// oscHandlerMethod().  Objects that want to register to receive OSC messages
// should inherit from OscHandler.

class OscHandler {
public:
    OscHandler(): oscController_(NULL) { }

    // The OSC controller will call this method when it gets a matching message
...
class OscController {
public:
    OscController(lo_server_thread thread, lo_address transmitAddr, const char* prefix):
        midiController_(NULL), useOscMidi_(false) {
    ...
    bool addListener(string& path,
                     OscHandler* object); // Add a listener object for a specific path
    bool removeListener(string& path,
                        OscHandler* object); // Remove a listener object

    // staticHandler() is called by liblo with new OSC messages.  Its only
    // function is to pass control to the object-specific handler method, which
    // has access to all internal variables.

    int handler(const char* path, const char* types, lo_arg** argv, int argc, lo_message msg, void* data);
    static int staticHandler(const char* path, const char* types, lo_arg** argv, int argc, lo_message msg,
                             void* userData) {
        return ((OscController*)userData)->handler(path, types, argv, argc, msg, userData);
    }

    // This method allows classes to transmit their own OSC messages.  The
    // variables in "..." should conform to the types specified, and should
    // terminate with LO_ARGS_END.
...
private:
    // MIDI emulation
    int handleMidi(unsigned char byte1, unsigned char byte2,
                   unsigned char byte3); // Handle OSC-encapsulated MIDI
    ...
    pthread_mutex_t oscListenerMutex_; // This mutex protects the OSC listener
                                       // table from being modified mid-message

    multimap<string, OscHandler*> noteListeners_; // Map from OSC path name to handler (possibly multiple
                                                  // handlers per object)
};
