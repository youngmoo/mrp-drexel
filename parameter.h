...
// Set the number of samples between ramped parameter updates.  This is roughly
// equivalent to csound's ksmps variable.

#define PARAMETER_UPDATE_INTERVAL 16

enum { shapeLinear = 0, shapeLogarithmic = 1, shapeStep = 2, shapeHold = 3 };
...
typedef struct {
    ...
    bool gated; // Whether this parameter changle waits for an external trigger
} parameterValue;
...
public:
    ...
    void gateTrigger(); // Trigger the next parameter value, if gated

private:
    void startRamp(); // Begin ramping to the next value

    double sampleRate_; // Need to know the sample rate to properly calculate the
                        // step size
    double updateRate_; // This value reflects the default parameter update
                        // interval (k-rate)
    ...
    bool gateTriggerReceived_; // Whether a trigger has come in for gated values
    bool waitingForGateTrigger_; // Whether we have hit the gate and are waiting
};
