...
void ButterBandpassFilter::clearBuffer() { bzero(history_, 2 * sizeof(float)); }
...
GenericFilter::GenericFilter(vector<double>& a, vector<double>& b) {
    ...
    if (bLength_ != 0) {
        ...
        for (i = 0; i < b.size(); i++)
            b_[i] = b[i];
    } else // Filter doesn't make sense without b coefficients-- make it a
           // passthrough
    {
        bLength_ = 1;
        b_ = new double[1];
...
EnvelopeFollower::EnvelopeFollower(float timeConstant, float sampleRate) {
    ...
    // So the output falls to 1/e in the span of one time constant.  From this and
    // the sample rate we can figure out what each sample needs to be multiplied
    // by.
