...
// This class implements a single note, which may control multiple synth
// objects.  Notes are triggered by the MIDI controller, which will also pass
// additional messages (note off, control change, etc.).

class Note {
public:
    // Constructor receives a reference to the MIDI Controller for various
    // callback purposes
    ...
    virtual int parseXml(TiXmlElement* baseElement) {
        return -1;
    } // Parse an XML file containing all relevant note settings
    ...
    void setPerformanceParameters(int audioChannel, int mrpChannel, int midiNote, int midiChannel, int pianoString,
        ...
        audioChannel_ = audioChannel; // Set the parameters that differ for each
    ...
    virtual void setResponseToPedals(bool damper, bool sostenuto, bool always) {
    } // Tells this note whether to continue playing when sustained
    ...
    //   begin(): start the note now; usually triggered by MIDI Note On, and
    //   immediately follows the constructor release(): usually called from MIDI
    //   Note Off.  Sound might stop, or there might be some kind of tail on this
    //   note releaseDamperDown(): a stronger form of release called when the
    //   damper on the string comes down.
    //				   In most cases the note should stop right
    // away.
    ...
    // This method is called from an external thread to check whether the note is
    // finished.  Should return true if all synths are done.  Each Note subclass
    // should override this to provide its own implementation, otherwise the note
    // will be terminated immediately.

    virtual bool isFinished() { return true; }

    // MIDI data methods: these are called whenever relevant MIDI data comes in.
    // The Note object can call a filter method in MidiController to set which
    // messages it wants to receive.  By default, these methods do nothing
...
protected:
    ...
    AudioRender* render_; // Reference to the AudioRender object which actually
                          // handles the rendering
    int audioChannel_; // Which DAC channel we use.  Guaranteed to be valid.
    int mrpChannel_; // Which input of the MRP controller this uses; might be
                     // different than audioChannel
    int midiNote_; // What MIDI note triggered this note (different, potentially,
                   // from both the piano string and the actual pitch we play)
    int midiChannel_; // Which MIDI channel this note triggers from (-1 for
                      // non-MIDI).
    int pianoString_; // Which piano string (by MIDI note #) this note routes to.
                      // Note that this might be different than the MIDI pitch we
                      // play
    double startTime_; // When this note started
    int priority_; // Higher numbers mean higher priority (for purposes of turning
                   // off when out of channels)
    unsigned int key_; // Internal variable to be used to find this note in
                       // MidiController's map
...
public:
    ...
    // release, releaseDamperDown, abort: tells all synths to release now, and
    // removes them from the queue
    //   eventually, notes with a tail will have a different release and
    //   releaseDamperDown method than abort()
    // cleanupIfFinished: check if all synths are done; if so, remove them from
    // the queue and ourselves from the controller

    void midiControlChange(unsigned char channel, unsigned char control, unsigned char value);

    // send a gate message to all listening synths
    void gateTrigger();
    ...
    MidiNote* createNote(int audioChannel, int mrpChannel, int midiNote, int midiChannel, int pianoString,
    ...
    static void parseMultiParameterRampWithVelocity(TiXmlElement* element, vector<timedParameter>* out1,
...
protected:
    ...
    vector<SynthBaseFactory*> factories_; // These are special containers that hold a range of possible
                                          // parameter values for each synth.  When createNote() is
                                          // called a new MidiNote is created which holds synths instead
                                          // of factories.

    int velocity_; // MIDI velocity number
    string name_; // The name of this patch
    bool sustainOnDamperPedal_,
        sustainOnSostenutoPedal_; // Whether or not this note continues if
                                  // prolonged by this pedal
    bool sustainAlways_; // Whether we always sustain beyond key release (until
                         // aborted externally)
    float velocityCurve_; // Curvature of velocity parameter, for transeg (0 =
                          // linear)
    bool isReleasing_; // Whether we're in releasing mode (i.e. waiting for a
                       // pedal change)

    // ***** Factory Classes ******
    // The idea here is to store the range of possible parameters a synth might
    // take, since we won't know the exact set of values until we know the MIDI
    // note number and velocity coming in.  These classes contain a method to
    // instantiate a new Synth with the correct values.

    typedef struct { // This holds the starting and ramping values of each
                     // parameter
        double start; // Saves using separate variables to hold them.  In synth.h,
                      // we use actual Parameter
        timedParameter ramp; // objects, but we don't want all that baggage here.
    } paramHolder;

    class SynthBaseFactory // Make all members public, but only MidiNote can see
                           // them since the class is private
    {
    public:
        ...
        // This creates a new object and returns it.  The object will eventually
        // need to be deleted elsewhere.
        virtual SynthBase* createSynth(int note, int velocity, float velocityCurvature, float amplitudeOffset) {
            return new SynthBase(sampleRate_);
        }
    ...
    protected:
        // This function calculates a user-defined curve similar to csound's transeg
        // opcode
    ...
    public:
        PllSynthFactory(MidiController* controller):
            SynthBaseFactory(controller),
            useAmplitudeFeedbackActive_(false),
            useInterferenceRejectionActive_(false),
            filterQActive_(false),
            loopFilterPoleActive_(false),
            loopFilterZeroActive_(false),
            relativeFrequencyActive_(false),
            globalAmplitudeActive_(false),
            loopGainActive_(false),
            amplitudeFeedbackScalerActive_(false),
            inputGainsActive_(false),
            inputDelaysActive_(false),
            harmonicAmplitudesActive_(false),
            harmonicPhasesActive_(false) { }
        ...
        bool useAmplitudeFeedbackActive_, useInterferenceRejectionActive_, filterQActive_, loopFilterPoleActive_,
            loopFilterZeroActive_;

        // Single ramping parameters
        paramHolder relativeFrequencyMin_,
            relativeFrequencyMax_; // Substitutes for centerFrequency until we know
                                   // MIDI note
        ...
        float relativeFrequencyConcavity_, globalAmplitudeConcavity_, loopGainConcavity_,
            amplitudeFeedbackScalerConcavity_;
    ...
    public:
        NoiseSynthFactory(MidiController* controller):
            SynthBaseFactory(controller),
            globalAmplitudeActive_(false),
            filterFrequenciesActive_(false),
            filterQsActive_(false),
            filterAmplitudesActive_(false) { }
...
//   one (2) It listens to a pair of MIDI controllers to change the phase offset
//   and amplitude of the current note (3) It can write these values to a
//   special XML file that is loaded on startup (4) It does not need any Synth
//   parameters; one internal synth is loaded automatically
...
public:
    ...
    CalibratorNote* createNote(int audioChannel, int mrpChannel, int midiNote, int midiChannel, int pianoString,
                               unsigned int key, int priority, int velocity, float phaseOffset, float amplitudeOffset);
...
private:
    ...
    unsigned int phaseControl_,
        amplitudeControl_; // Which controllers we listen to for adjusting phase
                           // and amplitude offset
    unsigned int phaseControlChannel_,
        amplitudeControlChannel_; // Channels for the above controllers

    // TODO: Auto-calibrator synth (but this will need to be better tested... do
    // this later)
};

#pragma mark class ResonanceNote

// This class implements a note that enhances the natural sympathetic vibrations
// on the piano.  When triggered for a particular string (usually in the bass
// register), it listens to subsequent note events and synthesizes overtones of
// the low string.
...
public:
    ...
    ResonanceNote* createNote(int audioChannel, int mrpChannel, int midiNote, int midiChannel, int pianoString,
                              unsigned int key, int priority, int velocity, float phaseOffset, float amplitudeOffset);
