...
// This method is called by the controller right before a note is performed, to
// set the performance-specific parameters.  Other parameters of the note might
// remain more-or-less the same from one MIDI note to the next, but we probably
// won't know these until the last minute.
...
// Thoughts: with this system, there is a granularity of attack time equal to
// the number of frames per buffer (at least for notes set to begin "now").  At
// any given instant, the render thread will be calculating audio for some time
// in the future, so clearly the note should begin as soon as possible. On the
// other hand, the amount of CPU time spent in the render is indeterminate
// (probably very small), so the only predictable behavior is to wait until the
// beginning of the next render loop to start the note.
...
// Tell the note to release now, but allow it extra time to handle any extra
// tail
...
SynthBase::~SynthBase() {
    // Nothing to do here, for now.  Possibly remove this synth from the render
    // list.
#ifdef DEBUG_ALLOCATION
    cout << "*** ~SynthBase\n";
#endif
...
void PllSynth::setLoopFilterPole(float loopFilterPole) {
    ...
    if (loopFilterZero_ >= 0.0)
        setLoopFilterPoleZero(loopFilterPole_,
                              loopFilterZero_); // Update coefficients
}
...
void PllSynth::setLoopFilterPoleZero(float loopFilterPole, float loopFilterZero) {
    ...
    // If pole is at the origin, can't have the scalar be 0 or the filter will
    // always return 0
    ...
    b.push_back(-poleZeroRatio * (1.0 - piDivSampleRate * loopFilterZero)
                / (1.0 + piDivSampleRate * loopFilterZero)); // b[1]
    a.push_back(-(1.0 - piDivSampleRate * loopFilterPole)
                / (1.0 + piDivSampleRate * loopFilterPole)); // a[1] (a[0] = 1.0 always)
...
void PllSynth::setUseAmplitudeFeedback(bool useAmplitudeFeedback) {
    ...
    if (useAmplitudeFeedback_) {
        ...
        // For either amplitude feedback or interference rejection, we need this
        // envelope follower. Once it's initialized, nothing further to do with it.
        ...
        // If we have multiple harmonics, we may need to create filters and
        // followers from them too
...
#endif
            // Use size as a proxy for which harmonic we just added.  Notice that this
            // has to come AFTER the harmonicInputFilters_.push_back() call above, or
            // the calcluation will change.
...
void PllSynth::setInputGains(vector<double>& currentInputGains, vector<timedParameter>& rampInputGains) {
    // Use the currentInputGains size as our metric.  If rampInputGains.size is
    // greater, ignore the extra.  If it is smaller, don't ramp the last
    // parameters.
    size_t size = currentInputGains.size();
...
void PllSynth::setInputDelays(vector<double>& currentInputDelays, vector<timedParameter>& rampInputDelays) {
    // Use the currentInputDelays size as our metric.  If rampInputDelays.size is
    // greater, ignore the extra.  If it is smaller, don't ramp the last
    // parameters.
    size_t size = currentInputDelays.size();
...
void PllSynth::setHarmonicAmplitudes(vector<double>& currentHarmonicAmplitudes,
    ...
    while (size > harmonicAmplitudes_.size()) {
        ...
        if (useAmplitudeFeedback_) {
            // There will always be one fewer filterBuffers than harmonics, since the
            // main buffer is separate
            ...
            // Use size as a proxy for which harmonic we just added.  Notice that this
            // has to come AFTER the harmonicAmplitudes_.push_back() call above, or
            // the calcluation will change.
...
void PllSynth::setHarmonicPhases(vector<double>& currentHarmonicPhases, vector<timedParameter>& rampHarmonicPhases) {
    // Use the currentHarmonicAmplitudes size as our metric.  If
    // rampHarmonicAmplitudes.size is greater, ignore the extra.  If it is
    // smaller, don't ramp the last parameters.
...
void PllSynth::setAmplitudeFeedbackScaler(double currentScaler, timedParameter& rampScaler) {
    if (!useAmplitudeFeedback_) // This parameter doesn't mean anything without
                                // amplitude feedback
        return;
...
// Append methods add new ramp values at the end of the current scheduled
// changes

void PllSynth::appendInputGains(vector<timedParameter>& inputGains) {
    size_t size = inputGains.size();
...
void PllSynth::appendHarmonicAmplitudes(vector<timedParameter>& harmonicAmplitudes) {
    ...
    while (size > harmonicAmplitudes_.size()) {
        ...
        if (useAmplitudeFeedback_) {
            ...
            // Use size as a proxy for which harmonic we just added.  Notice that this
            // has to come AFTER the push_back() call above, or the calcluation will
            // change.
...
void PllSynth::gateTrigger() {
    // Trigger the gate on every Parameter
    if (centerFrequency_ != NULL)
        centerFrequency_->gateTrigger();
    if (loopGain_ != NULL)
        loopGain_->gateTrigger();
    if (phaseOffset_ != NULL)
        phaseOffset_->gateTrigger();
    if (globalAmplitude_ != NULL)
        globalAmplitude_->gateTrigger();
    for (int i = 0; i < harmonicAmplitudes_.size(); i++)
        if (harmonicAmplitudes_[i] != NULL)
            harmonicAmplitudes_[i]->gateTrigger();
    for (int i = 0; i < harmonicPhases_.size(); i++)
        if (harmonicPhases_[i] != NULL)
            harmonicPhases_[i]->gateTrigger();
}
...
int PllSynth::render(const void* input, void* output, unsigned long frameCount,
    ...
    for (i = 0; i < lastFrame; i++) {
        float inputSample = 0; // Delay-and-sum on multiple inputs (partially implemented right now)
        float vcoFrequency;
        float followerMain;
        ...
        if (sampleNumber_ % PARAMETER_UPDATE_INTERVAL == 0) {
            ...
            if (centerFrequency_->ramp(PARAMETER_UPDATE_INTERVAL) && loopGain_->currentValue() != 0.0) {
                // If centerFrequency_ changes, need to update the filter buffer
                // coefficients
                float freq = centerFrequency_->currentValue();
                float freqDivQ = freq * filterQinverse_; // Save some multiplies...
                ...
                if (useAmplitudeFeedback_) {
                    for (size_t j = 0; j < harmonicInputFilters_.size(); j++)
                        harmonicInputFilters_[j]->updateCoefficients(freq * (float)(j + 2), freqDivQ * (float)(j + 2));
                }
            }

            // Should we somehow condense these so we don't have to call so many ramp
            // functions?
        ...
        if (loopGain_->currentValue() != 0.0) {
            if (loopGainWasZero_) // Have to go back and update the stuff we skipped
                                  // over before
            {
                float freq = centerFrequency_->currentValue();
                float freqDivQ = freq * filterQinverse_; // Save some multiplies...
            ...
            if (numInputChannels_ > 0 && inBuffer != NULL) {
                if (!usingDelayAndSum_) // Bypass the delay and sum code if we don't
                                        // need it.
                    inputSample = *inBuffer; // Channel 1, gain 1.0
                else {
                    inputSample = 0.0;

                    for (size_t j = 0; j < min(inputGains_.size(), numInputChannels_); j++) {
                        // FIXME: inputDelays not yet implemented.  Need to keep long
                        // storage buffers for any channels that are delayed.
            ...
            if (useInterferenceRejection_) {
                // Interference rejection scales the loop gain down when there's more
                // energy at adjacent semitones than at the desired frequency.  This
                // avoids the problem of PLL locking to the wrong frequency, which
                // results in a note that doesn't play properly.
                ...
                // Find the maximum interfering signal, then compare it to the target
                // signal
                ...
                if (sampleNumber_ % DEBUG_MESSAGE_SAMPLE_INTERVAL == 0) {
                    cout << "filters: main = " << inputFilteredCenter << "low = " << inputFilteredLow
                         << "high = " << inputFilteredHigh << endl;
                    cout << "followers: main = " << followerMain << " low = " << followerLow
                         << " high = " << followerHigh << endl;
                }

#endif
            }

            // PLL loop filter: the input to the PLL is a multiplier block of its last
            // output sample and the current input (which comes from the main bandpass
            // filter), which then goes through the loop filter

            float loopFilterOutput = (float)loopFilter_->filter((double)(pllLastOutput_ * inputFilteredCenter));

            // Should loopGain be scaled according to centerFrequency?  Currently the
            // relative frequency displacement is smaller at higher frequencies, but
            // maybe we only care about absolute displacement.
            ...
            // Calculate the PLL VCO output (a single sine wave without any of the
            // harmonic or phase offset information that we ultimately send to the
            // DAC).
            pllLastOutput_ = waveTable.lookupInterp(waveTableSine, pllPhase_);
        } else {
            // If the loop gain is zero, don't need any of the fancy BPF, PLL
            // processing, since the output will remain at centerFrequency no matter
            // what.  However, we should remember to still pass some data through the
            // filter buffers so we're not left with artifacts later.
        ...
        if (useAmplitudeFeedback_) {
            // Amplitude feedback records the intensity of each harmonic by filtering
            // the input signal.  harmonicAmplitudes holds the target values, and the
            // outputs are adjusted to match this.  Note that we can only make the
            // output amplitudes larger; if they're already too large, just turn off
            // that particular harmonic and wait for it to come down. Not a perfect
            // feedback strategy by any means, but it produces musical results.

            size_t size = min(harmonicInputFilters_.size(),
                              harmonicAmplitudes_.size() - 1); // sanity check

            // If there are N harmonics (i.e. N amplitudes), there will be N-1
            // elements in harmonicInputFilters. This is because the fundamental
            // frequency has an amplitude, but its input filter is already handled in
            // inputFilteredCenter followerMain.
            ...
            //            filter this through a first-order lowpass, to pull out any
            //            weird peaks from follower (is the above necessary?) scale by
            //            some constant-- overall we need to figure out the proper
            ...
            if (target != 0.0) {
                ...
                if (sampleNumber_ % DEBUG_MESSAGE_SAMPLE_INTERVAL == 0) {
                    // cout << "Fundamental: target = " << target << " follower = " <<
                    // followerMain << " output = " << outputLevel << endl;
                }
            }
            ...
            for (size_t j = 0; j < size; j++) {
                ...
                if (sampleNumber_ % DEBUG_MESSAGE_SAMPLE_INTERVAL == 0) {
                    cout << "Harmonic " << j + 1 << ": target = " << target << " follower = " << followerHarmonic
                         << " output = " << outputLevel << endl;
                }

                hPhase = (j < harmonicPhases_.size() ? harmonicPhases_[j]->currentValue() : 0.0);
                outSample += outputLevel
                    * waveTable.lookupInterp(waveTableSine, pllPhase_ * (float)(j + 1) + hPhase + phaseShift);
            }
        } else {
            // With no amplitude feedback, harmonic amplitudes refer directly to the
            // strength of each partial at the output

            for (size_t j = 0; j < harmonicAmplitudes_.size(); j++) {
                ...
                outSample += (float)(harmonicAmplitudes_[j]->currentValue())
                    * waveTable.lookupInterp(waveTableSine, pllPhase_ * (float)(j + 1) + hPhase + phaseShift);
                // outSample += (float)(harmonicAmplitudes_[j]->currentValue())
                //				* waveTable.lookup(waveTableSine,
                // pllPhase_*(float)(j+1)+hPhase);
            }
        }
...
#ifdef DEBUG_MESSAGES_EXTRA
        if (sampleNumber_ % DEBUG_MESSAGE_SAMPLE_INTERVAL == 0) {
            cout << "freq = " << vcoFrequency << ", inputLevel = " << followerMain << ", lastSample = " << outSample
                 << endl;
        }
#endif
...
NoiseSynth::NoiseSynth(const NoiseSynth& copy): SynthBase(copy) {
    ...
    // Copy over the vector parameters.  Fortunately, we shouldn't have NULL
    // pointers in the vectors.
...
void NoiseSynth::setFilterFrequencies(vector<double>& currentFrequencies, vector<timedParameter>& rampFrequencies) {
    // If the argument sizes don't match, use the smaller one.  (They should
    // always match)
    size_t size = min(currentFrequencies.size(), rampFrequencies.size());

    pthread_mutex_lock(&parameterMutex_);

    // Check if the vector we're appending has more elements than our internal
    // storage, and if so, increase our storage accordingly.  Use 1kHz as the
...
void NoiseSynth::setFilterQs(vector<double>& currentQs, vector<timedParameter>& rampQs) {
    // If the argument sizes don't match, use the smaller one.  (They should
    // always match)
    size_t size = min(currentQs.size(), rampQs.size());

    pthread_mutex_lock(&parameterMutex_);

    // Check if the vector we're appending has more elements than our internal
    // storage, and if so, increase our storage accordingly.  Use 10 as the
    // default starting Q
...
void NoiseSynth::setFilterAmplitudes(vector<double>& currentAmplitudes, vector<timedParameter>& rampAmplitudes) {
    // If the argument sizes don't match, use the smaller one.  (They should
    // always match)
    size_t size = min(currentAmplitudes.size(), rampAmplitudes.size());

    pthread_mutex_lock(&parameterMutex_);

    // Check if the vector we're appending has more elements than our internal
    // storage, and if so, increase our storage accordingly.  Use 0.0 as the
    // default starting amplitude
...
void NoiseSynth::updateFilters() {
    size_t minSize = min(min(filterFrequencies_.size(), filterQs_.size()), filterAmplitudes_.size());
    ...
    } else // Uh-oh, filters has too many elements.  This will cause trouble in
           // render() so fix ASAP!
    {
...
int NoiseSynth::render(const void* input, void* output, unsigned long frameCount,
    ...
    for (i = 0; i < lastFrame; i++) {
        ...
        if (sampleNumber_ % PARAMETER_UPDATE_INTERVAL == 0) {
            ...
            // Note: there must never be more filters than collections of (freq, Q,
            // amplitude) The set and append methods are responsible for enforcing
            // this
            for (j = 0; j < filters_.size(); j++) {
                bool changed;

                filterAmplitudes_[j]->ramp(PARAMETER_UPDATE_INTERVAL);
                changed = filterFrequencies_[j]->ramp(PARAMETER_UPDATE_INTERVAL)
                    || filterQs_[j]->ramp(PARAMETER_UPDATE_INTERVAL);

                if (changed) {
                    filters_[j]->updateCoefficients(filterFrequencies_[j]->currentValue(),
                                                    (filterFrequencies_[j]->currentValue())
                                                        / (filterQs_[j]->currentValue()));
                }
            }
        ...
        else // Output sample is a sum of all filter outputs
        {
            ...
            for (j = 0; j < filters_.size(); j++) // Multiply the output by the Q to keep total energy the same
                outSample +=
                    filters_[j]->filter(noise) * filterAmplitudes_[j]->currentValue() * filterQs_[j]->currentValue();
        }
...
ResonanceSynth::ResonanceSynth(float sampleRate): SynthBase(sampleRate) {
    ...
    // By default, amplitude 0.1 (-20dB) and 6dB / octave harmonic rolloff, .5 sec
    // decay at middle C
    globalAmplitude_ = new Parameter(0.1, sampleRate);
...
// Add a new harmonic to the list to render.  midiNoteKey is usually the midi ID
// of the note which triggered this harmonic, and frequency and amplitude tell
// us the initial values of the harmonic.
...
int ResonanceSynth::render(const void* input, void* output, unsigned long frameCount,
    ...
    for (i = 0; i < lastFrame; i++) {
        ...
        // Go through the map of current harmonics and add the contribution from
        // each to the output
        it = harmonics_.begin();
