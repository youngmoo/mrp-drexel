...
public:
    ...
    void setStreamInfo(PaStream* stream, int numInputChannels, int numOutputChannels, int numStreamOutputChannels,
                       float sampleRate, const vector<int>& channelsToUse, const vector<int>& downmixChannels = {});
    ...
    // Set MRP controller channels to skip
    void setMRPControllerChannelsToSkip(vector<int>& channelsToSkip);

    // Set the global output amplitude
    void setGlobalAmplitude(float amp) { globalAmplitude_ = amp; }

    // Tools for finding an open audio channel (allocated first-come,
    // first-served) Returns the channel used, or -1 if none available.  Save this
    // channel number for freeing later.  Only one object can use a channel at a
    ...
    // staticRenderCallback() is called by portaudio, and it in turn passes
    // control to the instance-specific renderCallback, which holds state
    // information about the stream and contains the list of synths to render.

    int renderCallback(const void* input, void* output, unsigned long frameCount,
                       const PaStreamCallbackTimeInfo* timeInfo, PaStreamCallbackFlags statusFlags);
    static int staticRenderCallback(const void* input, void* output, unsigned long frameCount,
                                    const PaStreamCallbackTimeInfo* timeInfo, PaStreamCallbackFlags statusFlags,
                                    void* userData) {
    ...
    // set an extra callback to call from the audio thread callback
    void setPollCallback(void (*pollCallback_)(void*), void* arg);
    ~AudioRender();
...
private:
    ...
    int numStreamOutputChannels_;
    vector<int> outputChannels_; // A list of channels we can use for output
    vector<int> downmixChannels_; // A list of channels we can actually use for output
    vector<float> outputBuffer_; // optionally used as an intermediate buffer
    vector<int> mrpControllerSkipChannels_; // A list of channels to skip on the
                                            // MRP controller
    float sampleRate_;
    ...
    /* List of synth processes to execute on each callback.  Use a map so we can
     remove specific synths later. */
    set<SynthBase*> synths_;

    /* Render list mutex ensures that render list doesn't change while it's being
     * iterated */
    pthread_mutex_t renderMutex_, channelMutex_;
    void (*pollCallback_)(void*);
    void* pollCallbackArg_;
};
