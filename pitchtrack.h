...
// This class handles all the central dispatching related to tracking an
// incoming pitch.  Note that the actual tracking takes place externally, with
// the messages arriving via OSC.  The functions performed by this class
// include:
...
private:
    ...
    float frequencyBuffer_[PITCHTRACK_BUFFER_SIZE]; // These hold the last N values
                                                    // of frequency and amplitude
    float amplitudeBuffer_[PITCHTRACK_BUFFER_SIZE]; // History is important when
                                                    // triggering notes
    float fractionalMidiNoteBuffer_[PITCHTRACK_BUFFER_SIZE];
    int pitchSampleCount_[128]; // How many samples have been within tolerance of
                                // each pitch
    int frequencyBufferIndex_, amplitudeBufferIndex_, fractionalMidiNoteBufferIndex_;

    // Triggering parameters
    int triggerPositiveSamples_,
        triggerTotalSamples_; // If X out of Y samples match a pitch, trigger that
                              // note
    float pitchToleranceSemitones_; // What fraction of a semitone a pitch can
                                    // different and still be considered a "match"
    float amplitudeThreshold_; // Minimum amplitude for a sample to count as
                               // positive
    ...
    bool notesTriggered_[128]; // Whether each note has been triggered before (for
                               // one-shot notes)
...
public:
    ...
    // Update this note with new pitch-tracking values; we may want to change
    // synth parameters here
    ...
    PitchTrackNote* createNote(int audioChannel, int mrpChannel, int midiNote, int inputMidiNote, int pianoString,
                               unsigned int key, int priority, int velocity, float phaseOffset, float amplitudeOffset);

private:
    // Private methods
    void assignPitchTrackSynthParameters(PitchTrackSynth* synth, paramHolder* relativeInputFreq,
...
// This class provides a simple sinusoid-generating synth (no loop gain) whose
// frequency and amplitude can be adjusted in real-time to an external input.
// Some of this code is borrowed from PllSynth.
...
private:
    ...
    Parameter* pitchFollowRange_; // How far (in frequency ratio) to deviate from
                                  // supposed center pitch
    Parameter* pitchFollowRatio_; // How closely to track the incoming pitch
    Parameter* maxGlobalAmplitude_; // Overall amplitude
    Parameter* inputGain_; // How much the input amplitude is scaled by (but never
                           // exceeding maxGlobalAmplitude)
    Parameter* harmonicCentroid_; // Adjustment to previously set harmonics (add
                                  // or multiply in frequency)
    ...
    double phase_; // Current phase of the main oscillator, from which all others
                   // are derived
    double lastFrequency_, lastAmplitude_; // Last input values from pitch tracker
    double minInputFrequency_,
        maxInputFrequency_; // The range of input frequencies that are "in range"
    ...
    pthread_mutex_t freqAmpMutex_; // Mutex to handle more frequent
                                   // frequency/amplitude updates
};
