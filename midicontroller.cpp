...
MidiController::MidiController(AudioRender* render) {
    ...
    bzero(inputControllers_,
          16 * 128 * sizeof(unsigned char)); // Set default values
    ...
    if (pthread_create(&cleanupThread_, NULL, cleanupLoop, this) != 0) {
        cerr << "Warning: Error creating cleanup thread!  Notes will not "
                "auto-terminate.\n";
    }
}

void MidiController::setA4Tuning(float tuning, float bassStretch, float trebleStretch, int stretchSplitPoint) {
    a4Tuning_ = tuning;

    if (stretchSplitPoint < 0)
        stretchSplitPoint = 0;
    if (stretchSplitPoint > 128)
        stretchSplitPoint = 128;

    // One octave is normally 2.0 ratio
    // With a stretch ratio of N, it will instead be 2.0*N
    // Therefore, multiply frequency by 2^(stretchRatio*(noteDiff/12))
    // instead of 2^(noteDiff/12). = (2^stretchRatio)^(noteDiff/12)

    // Need to calculate tuning at stretch point relative to A4 tuning
    float tuningAtStretchPoint;
    if (stretchSplitPoint == 69)
        tuningAtStretchPoint = a4Tuning_;
    else if (stretchSplitPoint < 69) {
        // Split point belo A4; A4 is part of the treble stretch
        tuningAtStretchPoint =
            a4Tuning_ * (float)powf(powf(2.0, trebleStretch), ((float)stretchSplitPoint - 69.0) / 12.0);
    } else {
        tuningAtStretchPoint =
            a4Tuning_ * (float)powf(powf(2.0, bassStretch), ((float)stretchSplitPoint - 69.0) / 12.0);
    }

    // Bass strings are stretched down by bassStretch ratio
    for (int i = 0; i < stretchSplitPoint; i++)
        noteFrequencies_[i] =
            tuningAtStretchPoint * (float)powf(powf(2.0, bassStretch), ((float)i - stretchSplitPoint) / 12.0);

    // Treble strings are stretched up by trebleStretch ratio
    for (int i = stretchSplitPoint; i < 128; i++)
        noteFrequencies_[i] =
            tuningAtStretchPoint * (float)powf(powf(2.0, trebleStretch), ((float)i - stretchSplitPoint) / 12.0);
}
...
int MidiController::loadPatchTable(string& filename) {
    ...
    // Go through and load each patch.  Each one is enclosed in a <Patch> tag,
    // which has a name attribute and a class attribute.  Initialize a new member
    // of the given class and pass the rest of the parsing off to it, since it
    ...
    while (element != NULL) // Step through all elements of type Patch
    {
        ...
        if (patchName != NULL && patchClass != NULL) {
            ...
            cerr << "loadPatchTable() warning: <Patch> must have name and class "
                    "attributes, skipping\n";
    ...
    // Having loaded the patches themselves, now we need to load the patch table,
    // which maps MIDI Program numbers to patches.  The contents of the
    // <PatchTable> element tell us how to do this.
    ...
    {
        // This is something of an abuse of the MIDI Program Change specification in
        // that we change programs for multiple channels from a single Program
        // Change message.  However, for the immediate moment, it's important that
        // we define complete interfaces for the performer which encompass the
        // actions of both keyboards.

        int programId;

        if (element->QueryIntAttribute("id", &programId) != TIXML_SUCCESS) {
            programId = -1;
            // It's possible we have a non-numeric character here.  Match "+" to mean
            // "one higher than the last program"
            ...
            if (programId == -1)
                cerr << "loadPatchTable() warning: Could not find ID of program, "
                        "skipping.\n";
        }
            ...
            if (programId < 0 || programId > 127)
                ...
                while (channelElement != NULL) // Iterate through the channels
                {
                    ...
                    if (channelElement->QueryIntAttribute("id", &channelID) != TIXML_SUCCESS)
                        cerr << "loadPatchTable() warning: Could not find ID of channel, "
                                "skipping.\n";
                    else {
                        ...
                        if (channelElement->QueryIntAttribute("velocitySplitPoint", &info.velocitySplitPoint)
                            != TIXML_SUCCESS)
                        ...
                        while (patchElement != NULL) {
                            ...
                            if (text != NULL) {
                                if (info.velocitySplitPoint > 0) // If the velocitySplitPoint attribute is active,
                                                                 // choose different patches
                                { // based on note velocity
                                    ...
                                    if (velString != NULL) {
                                        ...
                                        useHighVelocity = useLowVelocity =
                                            true; // Save in both high and low velocity slots
                                } else
                                    useHighVelocity = useLowVelocity = true;
                                const string* auxString = patchElement->Attribute((const string) "aux");
                                if (auxString != NULL) // Step 1.33: check if we use the aux
                                                       // pedal to split patches
                                {
                                    bool b;
                                ...
                                if (noteToSave != NULL) {
                                    ...
                                    if (useAuxOff && useLowVelocity) {
                                        ...
                                        // cout << "Adding " << patchName << " to low velocity, aux
                                        // off\n";
                                    }
                                    if (useAuxOff && useHighVelocity) {
                                        info.notes[1] = noteToSave;
                                        // cout << "Adding " << patchName << " to high velocity, aux
                                        // off\n";
                                    }
                        ...
                        if (foundNote) {
                            // Step 2: Get damper and sostenuto pedal parameters.  These say
                            // whether the note responds to those pedals or not.  Both
...
#ifdef DEBUG_MESSAGES_EXTRA
                                cout << "loadPatchTable(): retriggerEachPress bool value read "
                                        "as '"
                                     << info.retriggerEachPress << "'\n";
#endif
                            }

                            // Step 2.5: Get note priority (optional parameter).  If not
                            // specified, use 0.
                            ...
                            if (text != NULL) {
                                // stringstream s(text->ValueStr());
                                // s >> info.priority;
                                info.priority = atoi(text->ValueStr().c_str());
                            }

                            // Step 2.75: Get note mono voice, if specified-- used to specify
                            // a group of notes
                            ...
                            // Step 2.8: Get the (optional) amplitude offset (volume
                            // adjustment) for this program, which can be either a single
                            // number or two numbers indicating a linear range from low to
                            // high.
                            ...
                            // Step 3: Get key range (optional parameter).  If not specified,
                            // use whole keyboard range
                            ...
                            if (noteNumbers.size() > 0) {
                                // TODO: set amplitudeOffset; find highest and lowest and adjust
                                // accordingly.
...
#ifdef DEBUG_MESSAGES_EXTRA
                                    cout << "loadPatchTable(): Added patch " << patchName << " to Ch" << channelID
                                         << " note " << (*it) << endl;
#endif
                                }
                            } else // If no range was defined, use the complete range
                            {
                                // TODO: set amplitudeOffset; if using a range, make it 21-108
                                // with values preserved on either side.
                                for (i = 0; i < 128; i++)
        ...
        // Look for elements telling us when to update the program (triggered by
        // keyboard activity).
        ...
        while (programChangeElement != NULL) {
            ...
            if (pcChannelString != NULL && pcNoteString != NULL & pcProgramString != NULL) {
                ...
                iChannel = atoi(pcChannelString->c_str());
                iNote = atoi(pcNoteString->c_str());
                // sChannel >> iChannel;
                // sNote >> iNote;
                ...
                if (iNote > 0) {
                    if (*pcProgramString == "+")
                        ...
                        iNewProgram = atoi(pcProgramString->c_str());
                    }
    ...
    // Parse the string map, which tells us which MIDI note should direct to which
    // string.  It isn't always a 1-1 mapping because there may not be actuators
    // on every string.  Presently, this is a global mapping that affects all
    // programs from all keyboards.
    ...
    if ((element = baseElement->FirstChildElement("StringMap")) != NULL) {
        ...
        while (element != NULL) {
            ...
            if (str != NULL && str2 != NULL) {
                ...
                {
                    cerr << "loadPatchTable() warning: In StringMap, note and string "
                            "range sizes don't match, skipping\n";
                } else {
                ...
                cerr << "loadPatchTable() warning: Range element missing 'note' or "
                        "'string' attributes\n";
            }
        ...
        while (controlElement != NULL) {
            ...
            if (controlName != NULL && controlId != NULL) {
                if (*controlName == (const string) "MasterVolume") {
                    ...
                    controlMasterVolume_ = atoi(controlId->c_str());
                    ...
                    if (controlThreshold != NULL) {
                        ...
                        controlPitchTrackInputMuteThresh_ = atoi(controlThreshold->c_str());
                    } else
                        controlPitchTrackInputMuteThresh_ = 16;

                    cout << "Pitch-Track Input Mute on controller " << controlPitchTrackInputMute_ << ", threshold "
                         << controlPitchTrackInputMuteThresh_ << endl;
                }
            }
...
void MidiController::allNotesOff(int midiChannel) // midiChannel == -1 ---> reset all channels
{
    ...
    while (it != currentNotes_.end()) {
        ...
        if (channel == midiChannel || midiChannel < 0) {
            ...
            if (midiChannel != -1)
                mrpSendRoutingMessage(oldNote->mrpChannel(),
                                      mrpOffValue_); // Disconnect the signal router
            oldNote->abort(); // Forcibly stop the note
            if (currentNotes_.count(key) > 0) // If the Note object has not removed itself during abort(), remove
                                              // it from the map
            {
...
{
    // Reset all controllers.  This may include the primary piano pedals, so we
    // need to handle those appropriately
    ...
    if (midiChannel < 0 || midiChannel > 15)
        bzero(inputControllers_,
              16 * 128 * sizeof(unsigned char)); // Set default values for all controllers
    else {
        // Question: does this zero out the right controller values?  How is the
        // array stored internally?
        bzero(inputControllers_[midiChannel],
              128 * sizeof(unsigned char)); // Set default values for this controller
    }
}

void MidiController::clearInputState() {
    // Delete all controller and patch information, but don't change the size of
    // the storage bins
    ...
    if (logController_ != 0) {
        ...
        message.push_back(MESSAGE_PROGRAM_CHANGE | (unsigned char)mrpChannel_); // Use channel 0
        message.push_back(currentProgram_);
        logController_->logMidiMessage(&message);
    }
...
// They lock the mutex to avoid any MIDI events interrupting execution;
// therefore they should never be called internally from the MIDI thread or
// deadlock will result.

void MidiController::consoleProgramChange(int program) { // Externally execute a program change (e.g. from the
                                                         // console)
...
int MidiController::consoleProgramIncrement() { // Increment the program by one
...
unsigned char MidiController::getPatchValue(int channel) {
    ...
    return currentProgram_; // For now, we use a system where every channel has
                            // the same program
}
...
void MidiController::setNoteDisabledChannels(
    vector<int>& channels) // Disable certain channels from triggering MIDI notes
{
    int i, ch;
...
void MidiController::rtMidiCallback(double deltaTime, vector<unsigned char>* message, int inputNumber) {
    if (message->size() == 0) // Do nothing
        ...
        // RtMidi will give us one MIDI command per callback, which makes processing
        // easier for us.
    ...
    if (command == MESSAGE_RESET) {
        // MIDI reset message, which we should respond to be clearing all input
        // state
    ...
    } else if (command < 0xF0) // Commands below this need to be filtered for channel
    {
        ...
        switch (command) // In each case, make sure the message is long enough
                         // before proceeding (sanity check)
        {
        case MESSAGE_NOTEON:
            // Logging handled in noteOn() and noteOff()
            if (message->size() < 3 || !canTriggerNoteOnChannel_[channel]) {
                pthread_mutex_unlock(&eventMutex_);
                return;
            }
            // First, tell anyone else who might be listening to this note
            // Send all note on events to anyone who's listening, since wanting to
            // know this kind of info is rare anyway.
            ...
            if ((*message)[2] == 0)
                ...
                // FIXME: this might be broken right now.
                checkForProgramUpdate((*message)[0] & 0x0F,
                                      (*message)[1]); // See if this event updates the program number
            }
            break;
        case MESSAGE_NOTEOFF:
            // Logging handled in noteOff()
            if (message->size() < 3 || !canTriggerNoteOnChannel_[channel]) {
                pthread_mutex_unlock(&eventMutex_);
                return;
            }
        ...
        case MESSAGE_CONTROL_CHANGE:
...
#ifdef DEBUG_MESSAGES
            cout << "Control change: channel " << channel << ", control " << (int)(*message)[1] << " = "
                 << (int)(*message)[2] << endl;
#endif
            ...
            if ((*message)[1] == controlMasterVolume_) {
                ...
                /*else
                {
                        ...
                        masterVolume = pow(10.0, -(127.0-(float)(*message)[2])/64.0);
                // About -40dB of attenuation max
                }*/
            ...
            } else if ((*message)[1] == 4 && (*message)[2] >= 64) {
                // FIXME: kludgy!
                cout << "gate trigger\n";
                ...
                for (it = currentNotes_.begin(); it != currentNotes_.end(); ++it) {
                    Note* note = it->second;
                    if (typeid(*note) != typeid(MidiNote) && typeid(*note) != typeid(RealTimeMidiNote))
                        continue;

                    ((MidiNote*)note)->gateTrigger();
                }
            }
            ...
            if ((*message)[1] == CONTROL_DAMPER_PEDAL
                && channel == 0) // Special treatment for damper pedal change on main piano
                damperPedalChange((*message)[2]);
            if ((*message)[1] == CONTROL_SOSTENUTO_PEDAL
                && channel == 0) // Similarly, special handling for piano sost. pedal
            ...
            // for(it = controlListeners_.begin(); it != controlListeners_.end();
            // it++)
            while (it != controlListeners_.end()) {
                // We send all control change messages for any channel to the notes.
                // This isn't standard MIDI behavior but the Note objects may want to
                // know, for example, about the status of the pedals on the main
                // keyboard.
...
// amplifiers (directionDown == true ---> the first amplifier is the highest
// string, else the first amplifier is the lowest string)
...
// Specific function to communicate with MRP signal-routing hardware, telling it
// to connect a particular input to a given piano string
...
// This method serves two purposes: first, it saves a reference to the OSC
// controller, which this class needs to pass it on to notes it creates. Second,
// it registers this object to receive OSC messages (for example, patch
// changes).
...
void MidiController::setOscController(OscController* c) {
    ...
    string harmonicPath("/ui/harmonic"), gatePath("/ui/gate");
    ...
    addOscListener(harmonicPath);
    addOscListener(gatePath);
}

// This method is called by the OscController when it receives a message we've
// registered for.  Returns true on success.
...
bool MidiController::oscHandlerMethod(const char* path, const char* types, int numValues, lo_arg** values, void* data) {
    ...
    if (!strcmp(path, "/pedal/damper") && numValues >= 1) // Manually control the damper pedal
    {
        ...
        // Notify any notes that want to receive control changes ( see
        // rtMidiCallback() )
        it = controlListeners_.begin();
        while (it != controlListeners_.end())
    ...
    if (!strcmp(path, "/ui/allnotesoff") && numValues >= 1) // Turn all current notes off
    {
        ...
        if (param >= 0.5) {
            ...
            if (pitchTrackController_ != NULL) {
                pitchTrackController_->allNotesOff();
            }
        }
        return true;
    }
    ...
    if (!strcmp(path, "/ui/harmonic") && numValues >= 1) {
        float val = 0;
        if (types[0] == 'i')
            val = values[0]->i;
        else if (types[0] == 'f')
        ...
        for (it = currentNotes_.begin(); it != currentNotes_.end(); ++it) {
            ...
            cout << "Updating harmonic for note " << note->midiNote() << " to " << val << endl;

            ((RealTimeMidiNote*)note)->setAbsoluteHarmonicBase(val);
            ((RealTimeMidiNote*)note)->updateSynthParameters();
        }
        pthread_mutex_unlock(&eventMutex_);

        char numberText[8];
        snprintf(numberText, 7, "%.2f", val);
        oscController_->sendMessage("/ui/harmonic/number", "s", numberText, LO_ARGS_END);
    }
    if (!strcmp(path, "/ui/gate") && numValues >= 1) {
        float param = 0;
...
// Send the current patch value out over OSC to the UI.  Returns true on
// success.
...
void MidiController::addEventListener(Note* note, bool control, bool aftertouch, bool pitchWheel, bool otherNotes) {
    // Add this Note object to the event listener sets for control changes,
    // aftertouch, pitchwheel, and other note on/off events
...
void MidiController::removeEventListener(Note* note) {
    // Remove this Note object from any listener sets.  If the note wasn't part of
    // the sets, does nothing
...
void MidiController::noteEnded(Note* note, unsigned int key) {
    ...
    // Note objects call this when they finish releasing.  (Note that abort()
    // might not call this.) We should remove the indicated note from the map of
    // current notes and return its audio channel to the pool. Finally, delete the
    // Note object.
    ...
    mrpSendRoutingMessage(note->mrpChannel(),
                          mrpOffValue_); // Disconnect the signal routing for this string

    if (currentNotes_.count(key) > 0) {
        // If the note is part of a mono voice, mark it as available
        // int monoVoice = currentNotes_[key].monoVoice_;
        // if(monoVoice >= 0 && monoVoice < 16) {
        //    monoVoiceNotes_[monoVoice] = -1;
        //}

        // TODO: remove from monoVoice list
...
bool MidiController::pianoDamperLifted(int note) {
    // In case we run into a continuous pedal controller (Disklavier?),
    // quarter-pedal is good enough for us.

    return ((inputControllers_[0][CONTROL_DAMPER_PEDAL] >= DAMPER_PEDAL_THRESHOLD)
            || (pianoDamperStates_[note] != DAMPER_DOWN));
}

// This static function performs a timed check for finished notes.  When it
// finds one, it tells the note to release.
...
void* MidiController::cleanupLoop(void* data) {
    ...
    while (!controller->cleanupShouldTerminate_) {
        ...
        while(it != controller->currentNotes_.end())
        {
                ...
                {
    #ifdef DEBUG_MESSAGES
                        cout << "Found finished note " << (*it).second << ",
    aborting\n"; #endif
                        (*it++).second->abort();
                }
                else
        ...
        // Check every 10 ms.  Notes that are finished won't be actively rendering
        // audio but they will be occupying a channel. This is a reasonable
        // compromise between responsiveness and overhead
        usleep(10000);
    }
...
void MidiController::noteOn(double deltaTime, vector<unsigned char>* message, int inputNumber) {
    ...
    // If this came from the piano keyboard, we should make a note that the key
    // was pressed so that we keep track of the state of the damper.
    ...
    // When a Note On message is received, we should instantiate a new Note object
    // and tell it to begin Add this note object to the map of currently sounding
    // notes so we know what to release when we get a Note Off message later.

    // First things first: let's check that this event actually corresponds to a
    // note!

    if (programs_.count(PROGRAM_ID(currentProgram_, midiChannel, midiNote)) == 0) {
#ifdef DEBUG_MESSAGES_EXTRA
        cerr << "Warning: no Note found for program " << currentProgram_ << ", channel " << midiChannel << ", note "
             << midiNote << endl;
#endif
        return;
    }
    ...
    if (currentNotes_.count(key) > 0) {
        ...
        if (!programs_[PROGRAM_ID(currentProgram_, midiChannel, midiNote)].retriggerEachPress) {
            // If the note is set not to retrigger on a duplicate press, let it still
            // sound and don't start a new note
            return;
        }
    ...
    // If this note is assigned to a monophonic voice, check if there is any other
    // note present in the voice, and if so, turn it off.
    ...
    // Now, allocate an audio channel for this note.  But only if we have the
    // space, or if there are lower-priority notes to turn off.
    ...
    if (channels.first == -1) // No channel available, or error occurred
    {
        ...
        // Compare the lowest priority in the current collection to the priority of
        // the note that we propose to turn on.
    ...
    // Send a MIDI output message (if we're using MIDI out) to the router
    // hardware, sending this note to the correct string
...
#ifdef DEBUG_MESSAGES_EXTRA
    cout << "Phase offset = " << phaseOffsets_[midiNote] << ", amplitude offset = " << amplitudeOffsets_[midiNote]
         << endl;
#endif

    // aux and velocitySplit will tell us which of the patches from this program
    // we should use
    ...
    cout << "currentProgram_ " << currentProgram_ << " channel " << midiChannel << " noteIndex " << noteIndex
         << " Note " << oldNote << endl;
    if (oldNote == NULL) {
    ...
    {
        // The note in the program table holds factory classes which will
        // instantiate the correct synths for this instance
        newNote = ((MidiNote*)oldNote)
                      ->createNote(channels.first, channels.second, midiNote, midiChannel, stringNoteMaps_[midiNote],
                                   key, priority, midiVelocity, phaseOffsets_[midiNote],
                                   amplitudeOffsets_[midiNote] * thisNoteAmplitudeOffset);
        newNote->setResponseToPedals(damper, sostenuto, sustainAlways);
    } else if (typeid(*oldNote) == typeid(RealTimeMidiNote)) // Check class type
    {
        // The note in the program table holds factory classes which will
        // instantiate the correct synths for this instance
        newNote = ((RealTimeMidiNote*)oldNote)
                      ->createNote(channels.first, channels.second, midiNote, midiChannel, stringNoteMaps_[midiNote],
                                   key, priority, midiVelocity, phaseOffsets_[midiNote],
                                   amplitudeOffsets_[midiNote] * thisNoteAmplitudeOffset, 0.0, 0.0, 0.0, 0.0);
        newNote->setResponseToPedals(damper, sostenuto, sustainAlways);
        ((RealTimeMidiNote*)newNote)->setOscController(oscController_); // This type responds to OSC messages
    } else if (typeid(*oldNote) == typeid(CalibratorNote)) {
        newNote = ((CalibratorNote*)oldNote)
                      ->createNote(channels.first, channels.second, midiNote, midiChannel, stringNoteMaps_[midiNote],
                                   key, priority, midiVelocity, phaseOffsets_[midiNote],
                                   amplitudeOffsets_[midiNote] * thisNoteAmplitudeOffset);
        newNote->setResponseToPedals(damper, sostenuto, sustainAlways);
        ...
        newNote = ((ResonanceNote*)oldNote)
                      ->createNote(channels.first, channels.second, midiNote, midiChannel, stringNoteMaps_[midiNote],
                                   key, priority, midiVelocity, phaseOffsets_[midiNote],
                                   amplitudeOffsets_[midiNote] * thisNoteAmplitudeOffset);
        newNote->setResponseToPedals(damper, sostenuto, sustainAlways);
    } else {
        cerr << "Unknown Note type for program " << currentProgram_ << ", channel " << midiChannel << ", note "
             << midiNote << endl;
        render_->freeOutputChannel(channels.first);
        return;
    }

    // Add the note to the map of currently playing notes.  The key is an int
    // containing both the channel and the note number, to ensure that each key is
    // unique.

#ifdef DEBUG_MESSAGES
    cout << "Note on: " << midiNote << " (vel " << midiVelocity << ")"
         << " output " << channels.first << "/" << channels.second << " (string ";
...
// For a given MIDI note, check if this triggers a program change.  This is
// called separately from noteOn() so that the PianoBar controller can be more
// selective in when it triggers this event.
...
void MidiController::noteOff(double deltaTime, vector<unsigned char>* message, int inputNumber) {
    ...
    // See if there's a note in the map that matches this Note Off message, and if
    // so, tell it to release

    unsigned int key = ((unsigned int)midiChannel << 8) + (unsigned int)midiNote;
    if (currentNotes_.count(key) > 0) {
        Note* note = currentNotes_[key];

        if (pianoDamperLifted(note->pianoString())) // Damper is still up, because of either
                                                    // damper or sost. pedal
        {
#ifdef DEBUG_MESSAGES
            cout << "Note " << midiNote << " released, but damper still up\n";
#endif
            note->release(); // Don't do anything else with it for now; we'll handle
                             // it when the damper goes up
        } else {
#ifdef DEBUG_MESSAGES
        ...
        // When the note actually terminates (as opposed to just beginning to
        // release), we have to remove it from the map, but we don't do that here.
    } else {
        // Do nothing, since there doesn't seem to be any note like this!
        // It's possible that our audio channels have filled up, and that's why
        // there's no note
#ifdef DEBUG_MESSAGES_EXTRA
        cout << "Warning: no note found for NoteOff message, note = " << (int)(*message)[1] << " (channel "
             << (int)((*message)[0] & 0x0F) << ")\n";
#endif
    }
}
...
void MidiController::damperPedalChange(unsigned char value) {
    // If the damper pedal was depressed and has been released, we need to alert
    // any notes that are using strings whose dampers will be coming down.

    if (inputControllers_[0][CONTROL_DAMPER_PEDAL] < DAMPER_PEDAL_THRESHOLD || value >= DAMPER_PEDAL_THRESHOLD)
        return; // No need to do anything if either the pedal was previous not
                // depressed, or if it's still depressed

    // Iterate through the current notes (usually max 16, so not that slow) to
    // identify any that need to be alerted
    map<unsigned int, Note*>::iterator it = currentNotes_.begin();
    ...
    while (it != currentNotes_.end()) {
        ...
        {
#ifdef DEBUG_MESSAGES
            cout << "Damper went down on string " << stringUsed << ", output " << ((*it).second)->audioChannel()
                 << endl;
#endif
...
void MidiController::sostenutoPedalChange(unsigned char value) {
    ...
    // If the sostenuto pedal is depressed when it previously has not been, any
    // strings whose dampers are currently up should be changed in state to
    // reflect the fact that they'll be held up by the sost. pedal after the key
    // is released.

    // On the other hand, if the sostenuto pedal is released when it was
    // depressed, any strings in the sost. pedal state should be updated.  Notes
    // that use these strings may need to be alerted.

    if (inputControllers_[0][CONTROL_SOSTENUTO_PEDAL] < SOSTENUTO_PEDAL_THRESHOLD
        && value >= SOSTENUTO_PEDAL_THRESHOLD) {
    ...
    } else if (inputControllers_[0][CONTROL_SOSTENUTO_PEDAL] >= SOSTENUTO_PEDAL_THRESHOLD
               && value < SOSTENUTO_PEDAL_THRESHOLD) {
...
#endif
                    ((*it++).second)->releaseDamperDown(); // Increment the iterator
...
vector<int> MidiController::parseRangeString(const string& inString) {
    ...
    // Parse a string of the format "1,2-4,3-10,11,12" containing individual
    // numbers and ranges of numbers.  The resulting vector will hold all the
    // integers included in this range.
    ...
    while ((ap = strsep(&str, ",; \t")) != NULL) {
        ...
        if (strchr(ap, '-') != NULL) {
            ...
            // Next, this string might itself be a range of numbers (e.g. "5-10") so
            // we should separate by dashes
