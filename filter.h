...
// Second-order Butterworth bandpass filter, whose coefficients can be updated
// in real-time
...
public:
    ...
    void updateCoefficients(float frequency,
                            float bandwidth); // Update the filter coefficients
...
private:
    ...
    float a_[5];
    float history_[2]; // Need two back-samples of memory to calculate 2nd-order
                       // filter
};

// Generic filter structure, coefficients can be changed but this clears all
// state information (i.e. this is not well-suited to ramps, etc).  We use this
// for the PLL loop filter

class GenericFilter {
public:
    GenericFilter(vector<double>& a,
                  vector<double>& b); // Don't even need the sample rate here
...
private:
    ...
    size_t aLength_, bLength_;
};

// An envelope follower acts as an ideal full-wave rectifier followed by a
// parallel resistor and capacitor: Peaks in the signal are followed exactly,
// with a first-order decay between peaks.
