...
int RealTimeMidiNote::parseXml(TiXmlElement* baseElement) {
    // Here, call the MidiNote superclass to get all the regular parameters
    // (maybe).  But then we also want the following:
    ...
    if (rtqElement != NULL) {
        ...
        while (qualityElement != NULL) {
            ...
            if (name == NULL) {
                cerr << "RealTimeMidiNote::parseXml(): Quality object with no name, "
                        "skipping\n";
            ...
            if (name->compare("Intensity") == 0) {
                ...
                if (intensity_->parseXml(qualityElement)) {
                    cerr << "RealTimeMidiNote::parseXml(): Error parsing Intensity "
                            "quality.\n";
                }
                ...
                if (brightness_->parseXml(qualityElement)) {
                    cerr << "RealTimeMidiNote::parseXml(): Error parsing Brightness "
                            "quality.\n";
                }
                ...
                if (harmonic_->parseXml(qualityElement)) {
                    cerr << "RealTimeMidiNote::parseXml(): Error parsing Harmonic "
                            "quality.\n";
                }
            } else {
        ...
        if (qualityElement != NULL) {
            ...
            if (delay != NULL) {
                ...
                keyDownHoldoffTime_ = atof(delay->c_str());
            }
            if (scaler != NULL) {
                // stringstream scalerStream(*scaler);
                // scalerStream >> keyDownHoldoffScaler_;
                keyDownHoldoffScaler_ = atof(scaler->c_str());
            }
        ...
        if (qualityElement != NULL) {
            ...
            if (range != NULL) {
                ...
                harmonicSweepRange_ = atof(range->c_str());
            } else
            ...
            if (spread != NULL) {
                ...
                harmonicSweepSpread_ = atof(spread->c_str());
            } else
        ...
        cerr << "RealTimeMidiNote::parseXml(): no RealTimeQualities element found "
                "for Note "
             << name_ << endl;
    }
...
// Just like the superclass, but use a RealTimeMidiNote object instead.  Also
// set up the initial quality values.
...
RealTimeMidiNote* RealTimeMidiNote::createNote(int audioChannel, int mrpChannel, int midiNote, int midiChannel,
                                               ...
                                               float phaseOffset, float amplitudeOffset, double attack,
                                               double intensity, double brightness, double pitch) {
    ...
    // Copy the qualities from this object (which have been loaded by parseXml) to
...
void RealTimeMidiNote::updateSynthParameters() {
    // Here we "render" the internal parameter values of each quantity, combining
    // them into one set that goes to the synth.
    ...
    bool amplitudeUpdated = false, relativeFrequencyUpdated = false, loopGainUpdated = false,
         harmonicAmplitudesUpdated = false;
    vector<double> startHarmonicAmplitudes, startHarmonicPhases, harmonicPhases;
    ...
    std::vector<double>
#ifdef SCALEHARMONICOPT
        &
#endif // SCALEHARMONICOPT
            harmonicAmplitudes =
                intensity_->scaleHarmonicAmplitudes(startHarmonicAmplitudes, &harmonicAmplitudesUpdated);
    ...
    // As long as we have at least one synth (which we assume is of type PllSynth)
    // we can update its parameters.  This may be more computational overhead than
    // necessary if nothing has changed since last update on most paramters.

    if (synths_.size() > 0) {
        timedParameter tp;
        vector<timedParameter> vtp;

        // cout << "set globalAmplitude to " << globalAmplitude << endl;
        // cout << "set centerFrequency to " << relativeFrequency * centerFrequency_
        // << endl; for(i = 0; i < harmonicAmplitudes.size(); i++) 	cout << "set
        // harmonicAmplitudes[" << i <<"] to " << harmonicAmplitudes[i] << endl;
...
int RealTimeMidiNote::PllSynthQuality::parseXml(TiXmlElement* baseElement) {
    ...
    while (element != NULL) {
        ...
        if (value != NULL && name != NULL) {
            ...
            // float values, others will be a list of floats. We don't look for a
            // phase offset parameter in the XML file, since that is separately
            // calibrated
            if (name->compare("RelativeFrequency") == 0) { // double, time-variant
                ...
                MidiNote::parseCommaSeparatedValuesWithVelocity(value->c_str(), &harmonicAmplitudesMin_,
                                                                &harmonicAmplitudesMax_); // Collect the starting values
                ...
                MidiNote::parseCommaSeparatedValuesWithVelocity(value->c_str(), &harmonicPhasesMin_,
                                                                &harmonicPhasesMax_); // Collect the starting values
    ...
    if (element == NULL) {
        ...
        if (weight != NULL) {
            ...
            // s >> vibratoWeight_;
            vibratoWeight_ = atof(weight->c_str());
        ...
        // 1.0 means no vibrato since it's the base of an exponent.  < 0.0 makes no
        // sense.
...
// Update the value of this quantity: this in turn updates the current value of
// each of its affected parameters.  No change is made to the synth until the
// RealTimeMidiNote explicitly does so using the scale...() methods below.
...
// The following methods accumulate the effects of each quality on the synth
// parameters.  They will be called sequentially on each Quality by the
// controlling RealTimeMidiNote.
...
#ifdef SCALEHARMONICOPT
vector<double>&
#else // SCALEHARMONICOPT
vector<double>
#endif // SCALEHARMONICOPT
RealTimeMidiNote::PllSynthQuality::scaleHarmonicAmplitudes(vector<double>& inHarmonicAmplitudes, bool* updated) {
#ifdef SCALEHARMONICOPT
    out.resize(0);
#else // SCALEHARMONICOPT
    vector<double> out;
    ...
    if (!useHarmonicCentroid_)
        return out;
    else // If we use the harmonic centroid parameter, shift all harmonics upward
         // and round to nearest bin
    {
#ifdef SCALEHARMONICOPT
        out2.resize(0);
#else // SCALEHARMONICOPT
        ...
        for (i = 0; i < out.size(); i++) {
            ...
            } else // num has a fractional component
            {
                ...
                if (out2.size() > (int)ceil(num) - 1) {
                    out2[(int)floor(num) - 1] +=
                        val * (ceil(num) - num); // e.g. 1.8 --> bin 1 (strength .2), bin 2 (strength .8)
...
void RealTimeMidiNote::setOscController(OscController* c) {
    if (c == NULL)
        return;

    string harmonicPath("/ui/harmonic");

    OscHandler::setOscController(c);

    // if(allowOscHarmonicControl_)
...
bool RealTimeMidiNote::oscHandlerMethod(const char* path, const char* types, int numValues, lo_arg** values,
                                        void* data) {
    /*if(!strcmp(path, "/ui/harmonic") && numValues >= 1)
    {
            //calibrateSetPhase(values[0]->f);
    cout << "Updating harmonic for note " << midiNote_ << " to " << values[0]->f
    << endl;

    setAbsoluteHarmonicBase(values[0]->f);
    updateSynthParameters();

    char numberText[8];
    snprintf(numberText, 7, "%.2f", values[0]->f);
    oscController_->sendMessage("/ui/harmonic/number", "s", numberText,
    LO_ARGS_END);
...
void RealTimeMidiNote::PllSynthQuality::updateParameters() {
    double rawValue = currentCombinedValue(); // Get the summed effect of base and
                                              // vibrato to control parameters
    double value;
    ...
    if (useGlobalAmplitude_) {
        ...
        // cout << "min " << globalAmplitudeMin_ << " max " << globalAmplitudeMax_
        // << " c " << globalAmplitudeConcavity_ << " val " << value << endl;
        currentGlobalAmplitude_ =
            transeg(globalAmplitudeMin_, globalAmplitudeMax_, globalAmplitudeConcavity_, value, globalAmplitudeLinear_);
    }
    if (useRelativeFrequency_) {
        value = relativeFrequencyAbsolute_ ? fabs(rawValue) : rawValue;

        // cout << "min " << relativeFrequencyMin_ << " max " <<
        // relativeFrequencyMax_ << " c " << relativeFrequencyConcavity_ << " val "
        // << value << endl;
        ...
        // cout << "value " << value << " current " << currentRelativeFrequency_ <<
        // endl;
    }
    if (useHarmonicAmplitudes_) {
        value = harmonicAmplitudesAbsolute_ ? fabs(rawValue) : rawValue;

        size_t size = min(harmonicAmplitudesMin_.size(), harmonicAmplitudesMax_.size());
    ...
    if (useHarmonicPhases_) {
        ...
        size_t size = min(harmonicPhasesMin_.size(), harmonicPhasesMax_.size());
        ...
        for (size_t j = 0; j < size; j++) {
            currentHarmonicPhases_[j] = transeg(harmonicPhasesMin_[j], harmonicPhasesMax_[j], harmonicPhasesConcavity_,
                                                value, harmonicPhasesLinear_);
        }
    }
    if (useHarmonicCentroid_) {
        value = harmonicCentroidAbsolute_ ? fabs(rawValue) : rawValue;

        double tempCentroid = transeg(harmonicCentroidMin_, harmonicCentroidMax_, harmonicCentroidConcavity_, value,
                                      harmonicCentroidLinear_);
...
double RealTimeMidiNote::PllSynthQuality::transeg(double outVal1, double outVal2, double concavity, double inVal,
                                                  bool linear) {
