...
ostream& operator<<(ostream& output, const Parameter& p) {
    output << "val_ = " << p.val_ << ", oldValue_ = " << p.oldValue_ << ", stepValue_ = " << p.stepValue_
           << ", shape = ";
    switch (p.shape_) {
    case shapeHold:
        output << "hold\n";
    ...
    // output << "    sampleRate_ = " << p.sampleRate_ << ", updateRate_ = " <<
    // p.updateRate_ << endl;
    output << "    rampList_: ";
...
Parameter::Parameter(double initialValue, float sampleRate) {
    ...
    stepValue_ = 0.0;
    shape_ = shapeHold;
    gateTriggerReceived_ = false;
    waitingForGateTrigger_ = false;
}
...
void Parameter::appendRampValues(timedParameter& rampValues) {
    ...
    if (shape_ == shapeHold) // If we're waiting, not in the middle of a current
                             // ramp, begin a new one
        startRamp();
}

bool Parameter::ramp(int numSamples) // Returns true if the parameter has changed
{
    if (waitingForGateTrigger_)
        startRamp();
    if (waitingForGateTrigger_)
        return false;
    ...
    if (sampleCount_ >= sampleMax_) {
        // Reached our target; set the new value to the target in case there was any
        // drift
        if (rampList_.size() > 0) {
    ...
    case shapeLinear:
        ...
        {
            val_ = oldValue_
                + (rampList_[0].nextValue - oldValue_) * (double)sampleCount_
                    / (double)(sampleMax_ == 0 ? 1 : sampleMax_);
        }
    ...
    case shapeLogarithmic:
        ...
        { // No divide by 0; use 0.001 (-60dB) in this case
            if (rampList_[0].nextValue == 0.0)
                val_ = oldValue_
                    * pow(0.001 / (oldValue_ == 0.0 ? 0.001 : oldValue_),
                          (double)sampleCount_ / (double)(sampleMax_ == 0 ? 1 : sampleMax_));
            else
                val_ = oldValue_
                    * pow(rampList_[0].nextValue / (oldValue_ == 0.0 ? 0.001 : oldValue_),
                          (double)sampleCount_ / (double)(sampleMax_ == 0 ? 1 : sampleMax_));
        }
        break;
    case shapeStep:
...
void Parameter::gateTrigger() {
    if (waitingForGateTrigger_) {
        cout << "gate trigger received\n";
        gateTriggerReceived_ = true;
    }
}
...
void Parameter::startRamp() {
    ...
    if (rampList_.size() == 0) // Hold this value indefinitely
    {
        ...
        // Check if parameter is gated, and if so, wait for a trigger
        if (rampList_[0].gated) {
            if (!gateTriggerReceived_) {
                // Waiting: hold value but don't update sampleMax so this gets called
                // repeatedly until the trigger does finally come in
                stepValue_ = 0;
                shape_ = shapeHold;
                if (!waitingForGateTrigger_)
                    cout << "waiting on gate\n";
                waitingForGateTrigger_ = true;
                return;
            }
            // Trigger received; on to next value
            gateTriggerReceived_ = false;
            waitingForGateTrigger_ = false;
        }
        ...
        case shapeLinear: // Linear (additive) steps
            ...
            break;
        case shapeLogarithmic: // Logarithmic (multiplicative) steps -- avoid
                               // dividing by 0!
        ...
        cout << ", duration = " << duration << ", sampleRate = " << sampleRate_ << ", sampleMax_ = " << sampleMax_
             << ", stepValue_ = " << stepValue_ << endl;
#endif
    }
}