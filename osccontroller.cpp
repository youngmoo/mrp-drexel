...
// Call this internal method to add a listener to the OSC controller.  Returns
// true on success.
...
// The main handler method for incoming OSC messages.  From here, we farm out
// the processing depending on the path.  In general all our paths should start
// with /mrp.  Return 0 if the message has been adequately handled, 1 otherwise
// (so the server can look for other functions to pass it to).
...
int OscController::handler(const char* path, const char* types, lo_arg** argv, int argc, lo_message msg, void* data) {
    ...
    if (useOscMidi_) // OSC MIDI emulation.  Also the most time-sensitive &
                     // frequent message so do this first.
    {
    ...
    // Check if the incoming message matches the global prefix for this program.
    // If not, discard it.
    ...
    // Now remove the global prefix and compare the rest of the message to the
    // registered handlers.
    multimap<string, OscHandler*>::iterator it;
    pair<multimap<string, OscHandler*>::iterator, multimap<string, OscHandler*>::iterator> ret;
    string truncatedPath = pathString.substr(globalPrefix_.length(), pathString.length() - globalPrefix_.length());
...
// Adds a specific object listening for a specific OSC message.  The object will
// be added to the internal map from strings to objects.  All messages are
// preceded by a global prefix (typically "/mrp").  Returns true on success.
...
// Removes a specific object from listening to a specific OSC message. If path
// is NULL, removes all paths for the specified object.  Returns true if at
// least one path was removed.
...
bool OscController::removeListener(string& path, OscHandler* object) {
    ...
    pthread_mutex_lock(&oscListenerMutex_); // Lock the mutex so no incoming
                                            // messages happen in the middle

    multimap<string, OscHandler*>::iterator it;
    pair<multimap<string, OscHandler*>::iterator, multimap<string, OscHandler*>::iterator> ret;

    // Every time we remove an element from the multimap, the iterator is
    // potentially corrupted.  Realistically there should never be more than one
    // entry with the same object and same path (we check this on insertion).
...
// Send an OSC message to the specified path, which will be preprended by the
// application's global path.  Returns true if the message was sent, false if an
// error occurred.
...
bool OscController::sendMessage(const char* path, const char* types, ...) {
    ...
    if (!ret)
        cerr << "Error in OscController::sendMessage -- lo_message_add_varargs "
                "failed.\n";
    else
...
// These functions handle an OSC message updating one of the qualities of a
// currently playing note. The note number and MIDI channel are specified,
// pointing to a currently playing note.  If no such note exists, nothing
// happens.
