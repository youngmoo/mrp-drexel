...
// saves plenty of memory allocations as long as scaleHarmonicAmplitudes is
// called by a single thread
#define SCALEHARMONICOPT

#define MAX_CENTROID_HARMONICS 16

class RealTimeMidiNote : public MidiNote, public OscHandler {
protected:
...
public:
    ...
    // Set the starting parameters (before real-time modulation) for this note. Do
    // this before begin().
    ...
    // A thought: can we make generic quality-to-parameter maps that look up in
    // some sort of table?  Behavior could be defined by XML and then we just turn
    // the knob to slide along a scale.
    ...
    void setRelativePitchBase(double relativeVal) { pitch_->setBaseValue(pitch_->currentBaseValue() + relativeVal); }
    // static double semitoneRatio(double numSemitones) {	// helper method to
    // convert # of semitones to a freq ratio 	return pow(2.0,
    // numSemitones/12.0);
    //}
    double getHarmonicBase() { return harmonic_->currentBaseValue(); }
    void setAbsoluteHarmonicBase(double absoluteVal) { harmonic_->setBaseValue(absoluteVal); }
    void setRelativeHarmonicBase(double relativeVal) {
        harmonic_->setBaseValue(harmonic_->currentBaseValue() + relativeVal);
    }

    double getIntensityVibrato() { // Intensity vibrato (aka tremolo) is a
                                   // periodic modulation
        return intensity_->currentVibratoValue();
    }
    void setAbsoluteIntensityVibrato(double absoluteVal) { // in the intensity parameter (above).  These
                                                           // methods take
        intensity_->setVibratoValue(absoluteVal);
    }
    void setRelativeIntensityVibrato(double relativeVal) { // values centered around zero to modulate the
                                                           // current level
    ...
    void setAbsolutePitchVibrato(double absoluteVal) { // Similarly zero-centered (+/-1 maps to range
                                                       // loaded from XML)
    ...
    double getHarmonicVibrato() { // Frequency vibrato is a periodic modulation in
                                  // frequency
        return harmonic_->currentVibratoValue();
    }
    void setAbsoluteHarmonicVibrato(double absoluteVal) { // Similarly zero-centered (+/-1 maps to range
                                                          // loaded from XML)
    ...
    /*float getPitchBend() { // Similar to frequency vibrato but with a possibly
    different return pitchBend_.currentValue();
    }
    void setAbsolutePitchBend(float absoluteVal);	// range and designed for slower
    control void setRelativePitchBend(float relativeVal);	*/

    // Since there is a great deal of overlap among the above parameters, we need
    // some methods to get the combined qualities of all of them.

    double getCurrentIntensity() { // Both of these sum the effects of base and
                                   // vibrato/bend
        return intensity_->currentCombinedValue();
    }
    double getCurrentPitch() { return pitch_->currentCombinedValue(); }
    double getCurrentHarmonic() { return harmonic_->currentCombinedValue(); }
    ...
    int harmonicSweepRange() {
        return harmonicSweepRange_;
    } // Whether we redefine keys above this one to change harmonics
    int harmonicSweepSpread() {
        return harmonicSweepSpread_;
    } // Adjusts difference in harmonics for multiple simultaneous notes
    ...
    // void setReleaseProfile(double decayRate, double vibratoDepth, double
    // vibratoRate, double vibratoPhase);	// Later!

    void updateSynthParameters(); // Call this when all qualities have been
                                  // updated to calculate a new collection of
                                  // synth parameters.  Usually this will be done
                                  // once per PianoBar buffer.
...
#pragma mark private class PllSynthQuality
    ...
    public:
        PllSynthQuality():
            useGlobalAmplitude_(false),
            useRelativeFrequency_(false),
            useHarmonicAmplitudes_(false),
            useHarmonicPhases_(false),
            useLoopGain_(false),
            useHarmonicCentroid_(false),
            currentBaseValue_(0.0),
            currentVibratoValue_(0.0),
            vibratoWeight_(0.0) { }
        ...
        double currentCombinedValue() { // Sum the effects of base and vibrato
            // cout << "base " << currentBaseValue_ << " vib " << currentVibratoValue_
            // << endl;
            if (vibratoWeight_ == 0)
        ...
        // These methods take in a current parameter value and update it as
        // necessary with its current values.  If this quality doesn't affect a
        // particular parameter (e.g. useGlobalAmplitude_ = false), returns the same
        // value it gets in.
        ...
        scaleHarmonicAmplitudes(vector<double>& inHarmonicAmplitudes, bool* updated = NULL);
    ...
    private:
        void updateParameters(); // Called by both setBaseValue and setVibratoValue
                                 // to update synth parameters
        double transeg(double outVal1, double outVal2, double concavity, double inVal, bool linear);

        // Variables indicating the mapping from this quality to each parameter of
        // the PllSynth.  bool variables indicate whether this quality affects a
        // given parameter.
        ...
        double vibratoWeight_; // How much of an effect vibrato has on the parameter
                               // value
        bool clipVibratoLower_, clipVibratoUpper_; // Whether to clip the effect of
                                                   // vibrato to a [0,1] range
    ...
    bool allowOscHarmonicControl_;

    PllSynthQuality* intensity_; // These update in real time according to the set
                                 // methods above
