...
class MidiController : public OscHandler {
    ...
    friend class ContinuousKeyController;
    friend class OscController;
    friend class CalibratorNote; // Declare this as friend so it can read/write
                                 // the calibration values
    friend void* cleanupLoop(void* data);

public:
    typedef struct { // Struct holding some basic data about each MIDI program
        Note* notes[4]; // One of these structs will exist for each key of each
                        // channel (up to 16*128) notes[0] => low vel / no aux;
                        // notes[1] => high vel / no aux; notes[2, 3] => with aux
                        // pedal
        bool useDamperPedal;
        ...
        int priority;
        int monoVoice; // Monophonic voice this note is assigned to (0-15, or -1 for
                       // none)
        float amplitudeOffset; // Volume adjustment
    } ProgramInfo;

    // FIXME: Really, we should replace all this program info business with some
    // giant tree structure that splits based on any number of parameters: program
    // #, note, channel, velocity, control values, phase of the moon, etc.

    typedef struct {
        MidiController* controller; // The specific object to which this message
                                    // should be routed
        RtMidiIn* midiIn; // The object which actually handles the MIDI input
        int inputNumber; // An index indicating which input this came from in the
                         // user-defined order
    } midiCallbackStruct;
    ...
    void setLogController(PerformanceLog* log) { logController_ = log; }

    // ************ XML Input/Output *******************

    int loadPatchTable(string& filename); // Load patch information from a file.
                                          // Returns 0 on success.

    // ************ Calibration *******************
    int loadCalibrationTable(string& filename); // Load calibration information from file.  Returns 0
                                                // on success.
    int saveCalibrationTable(string& filename); // Save current values to a file.
                                                // Returns 0 on success.
    ...
    void clearInputState(); // Clear stored controller and patch values, remove
                            // all listeners, all notes off

    // Call these only from the console thread, never from the MIDI thread, since
    // they lock the same mutex
    ...
    void setDisplaceOldNotes(bool d) {
        displaceOldNotes_ = d;
    } // Whether to remove old notes when we run out of channels

    // The static callback below is needed to interface with RtMidi; it passes
    // control off to the instance-specific function
    void rtMidiCallback(double deltaTime, vector<unsigned char>* message,
                        int inputNumber); // Instance-specific callback
    ...
    // Specific function to communicate with MRP signal-routing hardware, telling
    // it to connect a particular input to a given piano string

    void mrpSetMidiChannel(int channel) { mrpChannel_ = channel & 0xFF; } // Default value is 0
    void mrpSetBaseAndDirection(int firstString, bool directionDown,
                                int offValue); // For USB controller
    void mrpSendRoutingMessage(int inputNumber, int stringNumber);
    void mrpClearRoutingTable();

    // ************ OSC Methods *******************

    // Call this before any OSC-using notes are used (overrides OscHandler
    // implementation)
    ...
    // These methods are called by Note objects to register themselves for updates
    // to control change, aftertouch, and pitch wheel MIDI input data.
    ...
    void noteEnded(Note* note,
                   unsigned int key); // Called when a Note finishes to request
                                      // removal from the map

    // ********** Utility Methods *****************

    // Retune the default frequencies
    float a4Tuning() { return a4Tuning_; }
    void setA4Tuning(float tuning, float bassStretch, float trebleStretch, int stretchSplitPoint);
    ...
    // The cleanup thread regularly calls these functions to check whether any
    // notes have finished.  The static function passes control to the
    // instance-specific function, which polls the available notes to see whether
    // they've finished. If so, it aborts the given note.  This allows notes to
    // finish on timers rather than solely on events.
...
private:
    ...
    void damperPedalChange(unsigned char value); // Called when the damper pedal
                                                 // changes on any channel
    void sostenutoPedalChange(unsigned char value); // Called when the sostenuto
                                                    // pedal changes on any piano

    void allNotesOff(int midiChannel); // Turn off all sounding notes for channel
                                       // (-1 = all channels)
    ...
    PerformanceLog* logController_;
    int mrpChannel_; // Which MIDI channel the MRP controller listens on
    int mrpFirstString_; // MIDI note of the first amplifier
    bool mrpDirectionDown_; // Whether the amplifiers are in ascending or
                            // descending order
    int mrpOffValue_; // Value that we send to turn off a given channel

    float noteFrequencies_[128]; // Frequency for each MIDI note
    bool displaceOldNotes_; // Whether, when we run out of channels, the newest or
                            // oldest notes take priority

    unsigned char inputControllers_[16][128]; // 16 channels x 128 controllers
    // unsigned char inputPatches_[16];			// 16 channels x 1 patch
    // each
    // --- currently use a global program change system instead
    unsigned int inputPitchWheels_[16]; // 16 channels x 1 value (14 bits)
    unsigned int currentProgram_; // Global program # changed by MIDI program
                                  // change message on any channel
    bool canTriggerNoteOnChannel_[16]; // Whether this channel allows MIDI note
                                       // triggering

    unsigned char stringNoteMaps_[128]; // Which string each note is played on
    int pianoDamperStates_[128]; // Keep track of the damper state for each piano
                                 // string This **does not** include the effect of
                                 // the damper pedal We'll probably only use 88 of
                                 // these but this keeps it synced to MIDI note
                                 // number

    map<unsigned int, Note*> currentNotes_; // Holds the currently sounding
                                            // notes, for MIDI note-off purposes
    unsigned int monoVoiceNotes_[16]; // Which note is currently sounding in a
                                      // defined monophonic voice
    set<Note*> aftertouchListeners_; // Notes that want to be updated on aftertouch data
    set<Note*> pitchWheelListeners_; // Notes that want to be updated on pitch
                                     // wheel changes
    set<Note*> controlListeners_; // Notes that want to be updated on control changes
    set<Note*> noteListeners_; // Notes that want to be updated on other note
                               // on/off events

    pthread_t cleanupThread_; // Thread identifier that runs the cleanup loop
    pthread_mutex_t eventMutex_; // This ensures that different MIDI streams and cleanup
                                 // events happen atomically It is a fairly course-grained
                                 // control (the whole of a MIDI action takes place within),
                                 // but it ensures our data doesn't get corrupted by competing
                                 // events.
    bool cleanupShouldTerminate_; // Set this to true on exit to let the cleanup
                                  // thread end
    ...
    float phaseOffsets_[128]; // Phase offset (0-1) for the fundamental frequency
                              // of each note
    float amplitudeOffsets_[128]; // Amplitude adjustment (relative to 1.0) for
                                  // each note
    string lastCalibrationFile_; // Filename of the last loaded or saved
                                 // calibration file
    ...
    // ProgramInfo programs_[16][128];			// Information about
    // each MIDI program on a per-key basis
    map<unsigned int, ProgramInfo> programs_;
    map<unsigned int, unsigned int> programTriggeredChanges_; // Holds info on MIDI events causing a program
                                                              // change

    // ****** Global Function Controllers *********

    int controlMasterVolume_; // Master volume control ID
    int controlPitchTrackInputMute_,
        controlPitchTrackInputMuteThresh_; // Control ID to mute the input, when
                                           // below threshold
