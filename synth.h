...
public:
    ...
    virtual int render(const void* input, void* output, unsigned long frameCount,
    ...
    // Query whether the synth is still running, or whether it has finished its
    // rendering
    bool isRunning() { return isRunning_; }
    bool isFinished() { return isFinished_; }

    // Gate trigger (does nothing on base class)
    virtual void gateTrigger() { }

    // Destructor immediately terminates the note.  The note should always be
    // removed from the render list before its destructor is called, or the
...
protected:
    ...
    unsigned int sampleNumber_; // Sample counter between calls, which can wrap as
                                // necessary
 ...
 * filter to isolate the desired frequency, and a phase-locked loop to
 *synthesize a clean output signal.  The output waveform is continuously
 *variable as a sum of harmonics.
 *****************/
...
public:
    ...
    int render(const void* input, void* output, unsigned long frameCount, const PaStreamCallbackTimeInfo* timeInfo,
               PaStreamCallbackFlags statusFlags);

    // These parameters are time-invariant, so we don't use the Parameter
    // structure for them
    void setFilterQ(double filterQ);
    void setLoopFilterPole(float loopFilterPole);
    void setLoopFilterZero(float loopFilterZero);
    void setLoopFilterPoleZero(float loopFilterPole,
                               float loopFilterZero); // One or the other of these
    void setLoopFilterAB(vector<double>& loopFilterA,
                         vector<double>& loopFilterB); // methods...
    ...
    // immediately
    void setInputGains(vector<double>& currentInputGains, vector<timedParameter>& rampInputGains);
    void setInputDelays(vector<double>& currentInputDelays, vector<timedParameter>& rampInputDelays);
    ...
    void setHarmonicPhases(vector<double>& currentHarmonicPhases, vector<timedParameter>& rampHarmonicPhases);
    void setAmplitudeFeedbackScaler(double currentScaler, timedParameter& rampScaler);

    // These methods schedule new parameter additions at the end of the current
    // ones
...
private:
    ...
    bool useAmplitudeFeedback_; // If true, use feedback on output levels to
                                // enforce the desired amplitude
    bool useInterferenceRejection_; // If true, scales down the loop gain in the
                                    // presence of interfering signals a semitone
                                    // above or below
    ...
    ButterBandpassFilter* mainInputFilter_; // Amplitude feedback requires BPFs
                                            // and envelope followers
    ButterBandpassFilter* lowInputFilter_; // at any harmonics we want to use.
                                           // Interference rejection
    ButterBandpassFilter* highInputFilter_; // requires BPFs and followers at
    ...
    /* Internal variables */
    double pllPhase_; // Current phase of the main PLL, from which all others are
                      // derived

    bool usingDelayAndSum_; // Whether we're using multiple inputs, or false for
                            // one input with gain 1.0
    bool loopGainWasZero_; // State information on whether loop gain was zero the
                           // last time around
 ...
 * class NoiseSynth
 *
 * Send filtered noise to the output, using any number of bandpass filters of
 *adjustable Q, center frequency, and amplitude.  This class does not make use
 *of an audio input.
 *****************/
...
private:
    ...
    Parameter* globalAmplitude_; // Overall amplitude
    vector<Parameter*> filterFrequencies_; // Frequency, Q, and level for an
                                           // arbitrary number of BPFs
 ...
 *
 * Send a combination of input channels directly to the output channel.  The
 *inputs are each adjustable by amplitude and delay.
 *****************/
 ...
 *
 * Reinforces the natural overtones of a given string, depending on input from
 *other MIDI notes.
 *****************/
