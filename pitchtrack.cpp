...
void PitchTrackController::parseGlobalSettings(TiXmlElement* baseElement) {
    ...
    if (settingsElement != NULL) {
        ...
        // <TriggerTotalSamples>
        // <TriggerPositiveSamples> -- how many positive samples out of the total
        // trigger a note <PitchTolerance> -- within what fraction of a semitone a
        // frequency matches a given note
        // <AmplitudeThreshold> -- how strong the amplitude must be before a match
        // is triggered
        ...
        if (text != NULL) {
            ...
            // cout << "Trigger positive samples = " << triggerPositiveSamples_ <<
            // endl;
        }
...
// This method is called by MidiController when it encounters a PitchTrack tag.
// We load our own custom data from the given element (representing
// <PitchTrack>)
...
void PitchTrackController::parsePatchTable(TiXmlElement* pitchTrackElement, int programId) {
        ...
        if (text != NULL) {
            ...
            if (noteToSave != NULL) {
                if (typeid(*noteToSave) != typeid(PitchTrackNote)) {
                    cerr << "parsePatchTable() error: need PitchTrackNote type for "
                            "program "
                         << programId << endl;
                } else {
                    foundNote = true;
    ...
    if (noteNumbers.size() > 0) {
        ...
        cerr << "PitchTrackController::parsePatchTable warning: no range "
                "specified, skipping program "
             << programId << endl;
...
void PitchTrackController::allNotesOff() {
    ...
    while (it != pitchTrackCurrentNotes_.end()) {
        ...
        oldNote = (*it++).second;
        midiController_->mrpSendRoutingMessage(oldNote->mrpChannel(),
                                               midiController_->mrpOffValue_); // Disconnect the signal router
        oldNote->abort(); // Forcibly stop the note
        if (pitchTrackCurrentNotes_.count(key) > 0) // If the Note object has not removed itself during abort(), remove
                                                    // it from the map
        {
...
void PitchTrackController::noteEnded(PitchTrackNote* note, unsigned int key) {
    ...
    // We should remove the indicated note from the map of current notes and
    // return its audio channel to the pool. Finally, delete the Note object.
    ...
    midiController_->mrpSendRoutingMessage(
        note->mrpChannel(),
        midiController_->mrpOffValue_); // Disconnect the signal routing for this string

    if (pitchTrackCurrentNotes_.count(key) > 0) // If the Note object has not removed itself during abort(), remove it
                                                // from the map
    {
        pitchTrackCurrentNotes_.erase(key);

        // In this case, the key should be just the note ID; we'll make sure it's in
        // range just in case
        if (key >= 0 && key < 128) {
...
// Reset the info that says whether each note has been triggered.  This will
// allow once-only notes to sound again.  In general we'll want to do this every
// time MidiController's patch change is updated
...
void PitchTrackController::programChanged() {
    ...
    // Turn on any notes that are supposed to be enabled immediately in this
    // program
    if (notesToTurnOn_.count(midiController_->currentProgram_) > 0) {
        ...
        for (i = 0; i < notesOn.size(); i++) {
            ...
            if (programs_.count(program) > 0 && soundingNotePriorities_[noteId] < 0 && !notesTriggered_[noteId]) {
                ...
                // See if there are any coupled notes to trigger.  Couplings are
                // expressed in semitone offsets (i.e. -1 = one semitone lower, 12 = one
                // octave higher)
                for (int j = 0; j < programs_[program].coupledNotes.size(); j++) {
                    triggerNote(programs_[program].note, noteId + programs_[program].coupledNotes[j], noteId,
                                programs_[program].priority);
                }
                // If this was a once-only trigger, set the flag saying it was triggered
                // so it doesn't happen again (unless this patch is reloaded).
...
// Handle input samples of the format {frequency, amplitude}.  Frequency is in
// Hz, amplitude on a 0-1 scale. Return true on success.

bool PitchTrackController::handleOscPitch(const char* path, const char* types, int numValues, lo_arg** argv,
                                          void* data) {
    ...
    // Convert this frequency to a fractional MIDI note; this normalizes frequency
    // differences across octaves and makes it easier to compare later whether the
    // pitch tests have been met
    ...
    // Find the range of pitches that could conceivably match this note, but
    // first, do this with the note triggerTotalSamples_ ago, to remove its effect
    // from the accumulator
    oldNote = fractionalMidiNoteBuffer_[(fractionalMidiNoteBufferIndex_ + PITCHTRACK_BUFFER_SIZE - triggerTotalSamples_)
                                        % PITCHTRACK_BUFFER_SIZE];
    ...
    // It will always be the most recent sample that puts us over the threshold
    // for any given note, so we should examine how this note relates to the
    // previous history.  The range of interest is plus or minus the threshold
    // size.
    ...
    if (fractionalMidiNote > 0.0 && !inputMuted_) {
        ...
        for (i = lowestPitchMatch; i <= highestPitchMatch; i++) {
            ...
            if (pitchSampleCount_[i] >= triggerPositiveSamples_) {
                // Matched this pitch; trigger a new note if there's one in the program
                // table
#ifdef DEBUG_MESSAGES_EXTRA
                ...
                if (programs_.count(program) > 0 && soundingNotePriorities_[i] < 0 && !notesTriggered_[i]) {
                    ...
                    // See if there are any coupled notes to trigger.  Couplings are
                    // expressed in semitone offsets (i.e. -1 = one semitone lower, 12 =
                    // one octave higher)
                    for (int j = 0; j < programs_[program].coupledNotes.size(); j++) {
                        triggerNote(programs_[program].note, i + programs_[program].coupledNotes[j], i,
                                    programs_[program].priority);
                    }
                    // If this was a once-only trigger, set the flag saying it was
                    // triggered so it doesn't happen again (unless this patch is
                    // reloaded).
                    if (programs_[program].onceOnly)
    ...
    // Alert existing notes to changes in pitch track value in case they want to
    // do anything
    it = pitchTrackCurrentNotes_.begin();

    while (it != pitchTrackCurrentNotes_.end()) {
        // If the input is muted, don't suppress the message, because the timing
        // disruption may lead the notes to do funny things.  Instead, send zeroes.
...
bool PitchTrackController::handleOscMute(const char* path, const char* types, int numValues, lo_arg** argv,
                                         void* data) {
    if (types[0] == LO_FLOAT) {
        if (argv[0]->f >= 0.5) {
...
// Messages should be of type "/mrp/ptrk/..." to get here, and will contain two
// floats.  The first holds frequency, the second holds amplitude. Use this
// information to decide what notes to trigger, and pass along information to
// any notes that want to hear it.

bool PitchTrackController::oscHandlerMethod(const char* path, const char* types, int numValues, lo_arg** argv,
                                            void* data)
// int PitchTrackController::oscHandler(const char *path, const char *types,
// lo_arg **argv, int argc, void *data)
{
    ...
    cerr << "PitchTrackController::oscHandlerMethod(): found unknown path " << path << " with " << numValues
         << " args\n";
    return false;
}
...
// This function is called when a pitch match is found to one of the values in
// the program table. It allocates a new audio channel, freeing old notes if
// necessary, and starts a new note playing. inputMidiNote is used to set the
// frequency at which the note responds to the pitch tracker.  If this is a
// coupled note, inputMidiNote may be different than midiNoteId Returns 0 on
// success.
...
int PitchTrackController::triggerNote(PitchTrackNote* noteRef, int midiNoteId, int inputMidiNote, int priority) {
    ...
    if (channels.first == -1) // No channel available, or error occurred
    {
        ...
        for (i = 0; i < 128; i++) {
            // Valid candidates for turnoff are those that are on but which have equal
            // or lesser priority Compile a vector of all notes with the lowest (same)
            // priority, and from there we'll decide which one started first and thus,
    ...
    if (midiController_->mrpDirectionDown_ && midiController_->mrpFirstString_ - pianoString >= 0)
        midiController_->mrpSendRoutingMessage(channels.second, midiController_->mrpFirstString_ - pianoString);
    else if (pianoString - midiController_->mrpFirstString_ >= 0)
        midiController_->mrpSendRoutingMessage(channels.second, pianoString - midiController_->mrpFirstString_);
    ...
    newNote =
        noteRef->createNote(channels.first, channels.second, midiNoteId, inputMidiNote, pianoString, midiNoteId,
                            priority, 127, /* velocity not used */
                            midiController_->phaseOffsets_[midiNoteId], midiController_->amplitudeOffsets_[midiNoteId]);
    ...
    // Set flag to show this note is active, so it doesn't attempt to retrigger
    // the very next sample if the pitch is the same.
...
vector<int> PitchTrackController::parseCommaSeparatedValues(const string& inString) {
    ...
    // Parse a string of the format "0, 1, -1, 4" containing a list of int values,
    // and put the values into a vector Strings of the form "1,,,,,,3,4" are
    // acceptable-- empty values are ignored
...
PitchTrackNote::PitchTrackNote(MidiController* controller, AudioRender* render, PitchTrackController* ptController):
    MidiNote(controller, render) {
...
// Parse an XML data structure to get all the parameters we need.  This extends
// the MidiNote method to pick up some pitch-tracking-specific parameters
...
// This method is called with new values from the pitch tracker.  Update the
// synth parameters accordingly.
void PitchTrackNote::pitchTrackValues(float frequency, float amplitude) {
    int i;
    bool shouldRelease = true;

    // Update all the synths
    // Use this opportunity to see if synths have released, and if all have done
    // so, stop the note

    for (i = 0; i < synths_.size(); i++) {
        if (get_ptr_typeid(synths_[i]) != typeid(PitchTrackSynth))
            continue;
...
void PitchTrackNote::abort() {
    // Call PitchTrackController instead of MidiController to remove from list of
    // active notes
    ...
    isRunning_ = false;
    pitchTrackController_->noteEnded(this,
                                     key_); // Tell the controller we finished
}
...
PitchTrackNote* PitchTrackNote::createNote(int audioChannel, int mrpChannel, int midiNote, int inputMidiNote,
    ...
    out->setPerformanceParameters(audioChannel, mrpChannel, midiNote, 0 /* midiChannel */, pianoString, key, priority,
                                  velocity);
    ...
    // Create copies of the synths in the new object, setting their appropriate
    // center frequencies
...
void PitchTrackNote::assignPitchTrackSynthParameters(PitchTrackSynth* synth, paramHolder* relativeInputFreq,
                                                     paramHolder* relativeOutputFreq, TiXmlElement* element) {
    ...
    if (value != NULL && name != NULL) {
        ...
        if (name->compare("MaxDuration") == 0) { // double, time-invariant
            ...
            // Save this information separately and send it to the synth once we know
            // which note it triggers from
...
PitchTrackSynth::PitchTrackSynth(float sampleRate): SynthBase(sampleRate) {
    ...
    // Set defaults for changeable parameters, so we don't have to worry about
    // allocating Parameters later
...
PitchTrackSynth::PitchTrackSynth(const PitchTrackSynth& copy): SynthBase(copy) {
    ...
    // Initialize a new mutex.  Just because the copy was locked doesn't mean this
    // one should be.
    ...
    // Finally, copy over the vector parameters.  Fortunately, we shouldn't have
    // NULL pointers in the vectors.
...
void PitchTrackSynth::begin() {
    ...
    // We won't know the exact start time until the first render cycle, so we
    // can't actually call release() until then.
...
// These methods replace the current parameters with new ones, starting
// immediately
...
void PitchTrackSynth::setHarmonicAmplitudes(vector<double>& currentHarmonicAmplitudes,
    ...
    // Use the currentHarmonicAmplitudes size as our metric.  If
    // rampHarmonicAmplitudes.size is greater, ignore the extra.  If it is
    // smaller, don't ramp the last parameters.
    size_t size = currentHarmonicAmplitudes.size();

    pthread_mutex_lock(&parameterMutex_);

    // Check if the vector we're appending has more elements than our internal
    // storage, and if so, increase our storage accordingly.  Use 0 as the default
    // starting amplitude
...
void PitchTrackSynth::setHarmonicPhases(vector<double>& currentHarmonicPhases,
    ...
    // rampHarmonicAmplitudes.size is greater, ignore the extra.  If it is
    // smaller, don't ramp the last parameters.
    size_t size = currentHarmonicPhases.size();
...
void PitchTrackSynth::appendHarmonicAmplitudes(vector<timedParameter>& harmonicAmplitudes) {
    size_t size = harmonicAmplitudes.size();
...
void PitchTrackSynth::appendHarmonicPhases(vector<timedParameter>& harmonicPhases) {
    size_t size = harmonicPhases.size();
...
// This function is called by the external process that provides pitch-tracking
// data lastFrequency and lastAmplitude are used in the render loop.
...
// Render one buffer of output.  input holds the incoming audio data.  output
// may already contain audio, so we add our result to it rather than replacing.
...
int PitchTrackSynth::render(const void* input, void* output, unsigned long frameCount,
    ...
    // If the note was set to begin "now", calibrate its start time to the
    // beginning of this buffer We use the start time to calculate the offset from
    ...
    // If the note has been set to release, check whether that should happen
    // immediately, within this buffer, or later.  If later, then go about our
    ...
    for (i = 0; i < lastFrame; i++) {
        ...
        // A parameter needs ramping when there is at least one item in its
        // timedParameter deque.
        ...
        // Lock this mutex during the calculations that use lastFrequency and
        // lastAmplitude.  We expect that these values might change from one frame
        // to the next, so we don't want to use the coarser-grained parameter mutex.

        // FIXME: Render actually happens a good deal before the audio comes out,
        // and the timing isn't specified. The interaction of threads here might
        // produce somewhat strange results....
        ...
        // Calculate the output frequency, which follows the input unless the input
        // is out of range (or unless we've disabled this feature

        if (pitchFollowRatio_->currentValue() != 0.0 && frequencyInRange) {
            float outInRatio;

            // We want one semitone in to mean one semitone out in deviation, but that
            // will require scaling for the respective registers of the tones
            ...
            vcoFrequency = outputCenterFrequency_->currentValue()
                + (lastFrequency_ - inputCenterFrequency_->currentValue()) * pitchFollowRatio_->currentValue()
                    * outInRatio;
        } else
        ...
        //   (3) If pitchFollowRange != 1.0, then the amplitude rolls off linearly
        //   away from the
        //       center frequency.  That's so we don't get weird clicks and pops as
        //       the input signal passes in and out of range.
        ...
        if (minInputFrequency_ < maxInputFrequency_
            && inputGain_->currentValue() >= 0.0) // True if pitchFollowRange > 1.0
        {
            // Calculate the distance from centerFrequency to the current frequency,
            // and do a linear rolloff in amplitude.  In perceptual (logarithmic)
            // space, the rolloff will be the steepest toward the edges, which is what
            // we want.

            // FIXME: if this works, the dividers can be put into state variables and
            ...
            if (!frequencyInRange)
                ...
                if (lastFrequency_ <= inputCenterFrequency_->currentValue()) {
                    rawOutputAmplitude *= (lastFrequency_ - minInputFrequency_)
                        / (inputCenterFrequency_->currentValue() - minInputFrequency_);
                } else {
                    rawOutputAmplitude *= (maxInputFrequency_ - lastFrequency_)
                        / (maxInputFrequency_ - inputCenterFrequency_->currentValue());
                }
            }
        }
        ...
        // Harmonic amplitudes refer directly to the strength of each partial at the
        // output
        for (j = 0; j < harmonicAmplitudes_.size(); j++) {
            ...
            if (harmonicCentroid_->currentValue() == 1.0) {
                ...
                // Calculate the actual contributions of this harmonic based on the
                // centroid, which could shift or scale its frequency value.
                ...
                // val will most probably lie between two integers, in which case we
                // assign each of its neighbors a weighted sum.  Keep some sort of limit
                // on how many harmonics can be defined this way, so the system doesn't
                // get too too slow.
        ...
        if (sampleNumber_ % DEBUG_MESSAGE_SAMPLE_INTERVAL == 0) {
            ...
            cout << "freq = " << vcoFrequency << ", lastFreq = " << lastFrequency_
                 << ", output amp = " << filteredOutputAmplitude << endl;
        }
#endif
