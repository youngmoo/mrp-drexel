...
const char* kNoteNames[88] = { "A0",  "A#0", "B0",  "C1",  "C#1", "D1",  "D#1", "E1",  "F1",  "F#1", "G1",
...
/*MidiNote::MidiNote(const MidiNote& copy) : Note(copy),
velocity_(copy.velocity_), name_(copy.name_)
{
        ...
        for(int i = 0; i < copy.synths_.size(); i++)
        {
                ...
                {
                        PllSynth *s = new
PllSynth(*dynamic_cast<PllSynth*>(copy.synths_[i])); synths_.push_back(s);
                }
                else if(typeid(*copy.synths_[i]) == typeid(NoiseSynth))
                {
                        NoiseSynth *s = new
NoiseSynth(*dynamic_cast<NoiseSynth*>(copy.synths_[i])); synths_.push_back(s);
                }
                else
                {
                        cerr << "Warning: unknown synth type in MidiNote copy
constructor\n";
                }
        }
...
// Parse an XML data structure to get all the parameters we need.  This will
// generally be called when the patch table is first loaded, to make one
// reference instance of each patch.  These reference instances will be copied
// every time a new note is triggered, and the performance-specific data (MIDI
// note #, velocity, output channel, etc.) will be passed in at that time.

int MidiNote::parseXml(TiXmlElement* baseElement) {
    // Presently, we look for Synth tags within this element.  A note can have one
    // or more synths, each of which has a set of ramping parameters.
    int numSynths = 0;
    ...
    // Check for a <VelocityCurve> tag to tell us how to shape the incoming
    // velocity parameter. If none, use the default 0.0
    ...
    if (element != NULL) {
        ...
        if (text != NULL) {
            ...
            // s >> velocityCurve_;
            velocityCurve_ = atof(text->ValueStr().c_str());
    ...
    // Allow "a/b" ranges on values to specify min and max velocity; if only one
    // is given, a=b Use a "concavity" attribute to specify concavity (default is
    // 0) Eventually, allow a velocity pre-mapping
...
void MidiNote::setPerformanceParameters(int audioChannel, int mrpChannel, int midiNote, int midiChannel,
...
// Tells this note whether to continue sustaining if prolonged by either damper
// or sostenuto pedals. Check this information at release time to decide whether
// to call abort() or not.
void MidiNote::setResponseToPedals(bool damper, bool sostenuto, bool always) {
    sustainOnDamperPedal_ = damper;
    sustainOnSostenutoPedal_ = sostenuto;
    sustainAlways_ = always;
}
...
void MidiNote::release() {
    ...
    if (sustainAlways_) // If we always sustain, just return now and wait for a
                        // later abort() call
        return;

    if (!sustainOnDamperPedal_ && !sustainOnSostenutoPedal_) // If we don't respond to either pedal, finish
                                                             // up now
    {
        abort();
        return;
        ...
        // unsigned char sost = controller_->getControllerValue(0,
        // MidiController::CONTROL_SOSTENUTO_PEDAL);

        if (!((sustainOnDamperPedal_ && damper >= DAMPER_PEDAL_THRESHOLD) // If neither pedal is sustaining us...
              /*|| (sustainOnSostenutoPedal_ && sost >=
                 SOSTENUTO_PEDAL_THRESHOLD)))*/
              || (sustainOnSostenutoPedal_
                  && controller_->pianoDamperState(midiNote_) & MidiController::DAMPER_SOSTENUTO))) {
            abort(); // ...then finish up.
            return;
        }
    }

    // If this hasn't called abort(), we'll get MIDI control change messages until
    // eventually we do abort.
    controller_->addEventListener(this, true, false, false, false);
}

void MidiNote::releaseDamperDown() {
    if (!sustainAlways_)
        abort();
}
...
void MidiNote::abort() {
    ...
    // This setup doesn't allow synths any post-release activity since it removes
    // them right away
    for (int i = 0; i < synths_.size(); i++) {
        synths_[i]->release();
#ifdef DEBUG_MESSAGES_EXTRA
...
void MidiNote::gateTrigger() {
    // Send a trigger to all parameters of all active synths
    for (int i = 0; i < synths_.size(); i++) {
        synths_[i]->gateTrigger();
    }
}

void MidiNote::midiControlChange(unsigned char channel, unsigned char control, unsigned char value) {
    // Check for damper or sostenuto pedal changes, if we're in release mode. Stop
    // the note if the right pedal change occurs

    if (!isReleasing_) // Sanity check-- should only get here during release
                       // anyway
        return;
    ...
    if (control == MidiController::CONTROL_DAMPER_PEDAL && sustainOnDamperPedal_) {
        // Check if pedal has been released; if so, check whether sostenuto pedal is
        // still holding this note.
        if (value >= DAMPER_PEDAL_THRESHOLD)
            return;
        if (sustainOnSostenutoPedal_
            && controller_->getControllerValue(0, MidiController::CONTROL_SOSTENUTO_PEDAL) >= SOSTENUTO_PEDAL_THRESHOLD)
            return;

        abort(); // Otherwise, the damper pedal is now released and the note should
                 // end
        return;
        ...
        if (value >= SOSTENUTO_PEDAL_THRESHOLD)
            return;
        // Given that sostenuto pedal has been released, this note should end even
        // if the pedal comes back down later. On the other hand, keep sustaining it
        // if it also responds to the damper pedal
        sustainOnSostenutoPedal_ = false;
        if (sustainOnDamperPedal_
            && controller_->getControllerValue(0, MidiController::CONTROL_DAMPER_PEDAL) >= DAMPER_PEDAL_THRESHOLD)
            return;
...
// This method creates and returns a copy of the object, but instead of
// containing factories, it contains real Synth objects with the right
// parameters for the particular MIDI note and velocity.

MidiNote* MidiNote::createNote(int audioChannel, int mrpChannel, int midiNote, int midiChannel, int pianoString,
    ...
    out->setPerformanceParameters(audioChannel, mrpChannel, midiNote, midiChannel, pianoString, key, priority,
                                  velocity);
    ...
    // Using the factory classes, create synths that reflect the specific
    // parameters of this note
...
// Private utility method that assigns parameter values to a synth based on XML
// data

void MidiNote::assignPllSynthParameters(PllSynthFactory* factory, TiXmlElement* element) {
    // Each Parameter holds three attribute tags: name, value, and (optionally)
    // concavity for velocity-sensitive parameters
    ...
    if (value != NULL && name != NULL) {
...
#endif
        if (concavity != NULL) // Read the concavity attribute if present; if not,
                               // assume linear (0)
        {
        ...
        // First, check that the name is a parameter we recognize.  Some will have
        // float values, others will be a list of floats. We don't look for a phase
        // offset parameter in the XML file, since that is separately calibrated
        if (name->compare("FilterQ") == 0) { // double, time-invariant
            ...
            parseVelocityPair(value->c_str(), &(factory->relativeFrequencyMin_.start),
                              &(factory->relativeFrequencyMax_.start));
            parseParameterRampWithVelocity(element, &(factory->relativeFrequencyMin_.ramp),
                                           &(factory->relativeFrequencyMax_.ramp));
            ...
            parseVelocityPair(value->c_str(), &(factory->globalAmplitudeMin_.start),
                              &(factory->globalAmplitudeMax_.start));
            parseParameterRampWithVelocity(element, &(factory->globalAmplitudeMin_.ramp),
                                           &(factory->globalAmplitudeMax_.ramp));
            ...
            parseMultiParameterRampWithVelocity(element, &vtp1,
                                                &vtp2); // and the ramping values
            ...
            parseVelocityPair(value->c_str(), &(factory->amplitudeFeedbackScalerMin_.start),
                              &(factory->amplitudeFeedbackScalerMax_.start));
            parseParameterRampWithVelocity(element, &(factory->amplitudeFeedbackScalerMin_.ramp),
                                           &(factory->amplitudeFeedbackScalerMax_.ramp));
...
timedParameter MidiNote::parseParameterRamp(TiXmlElement* element) {
    ...
    while (ramp != NULL) // These tags hold ramp values for time-variant parameters
    {
        ...
        const string* gated = ramp->Attribute((const string) "gated");
        ...
        if (rampVal != NULL && rampDur != NULL && rampType != NULL) {
            ...
            if (gated == NULL) {
                ...
                if (*gated == "true")
                    ...
                    param.gated = false;
                cerr << "parseParameterRamp(): gated = " << param.gated << endl;
            }

            out.push_back(param);
        } else
            cerr << "assignPllSynthParameters() warning: null attributes in Ramp "
                    "tag\n";
...
// Parse a series of <Ramp> elements whose values and durations may be of "a/b"
// format, storing the results in out1 and out2.
...
void MidiNote::parseParameterRampWithVelocity(TiXmlElement* element, timedParameter* out1, timedParameter* out2) {
    ...
    while (ramp != NULL) // These tags hold ramp values for time-variant parameters
    {
        ...
        if (rampVal != NULL && rampDur != NULL && rampType != NULL) {
            ...
            param1.gated = param2.gated = false;

            if (parseVelocityPair(rampVal->c_str(), &param1.nextValue, &param2.nextValue) == 0)
                cerr << "parseParameterRampWithVelocity() warning: found no valid "
                        "values, using 0\n";
            ...
            if (gated == NULL) {
                ...
                if (*gated == "true")
                    param2.gated = param1.gated = true;
                else
                    param2.gated = param1.gated = false;
                cerr << "parseParameterRampWithVelocity(): gated = " << param1.gated << endl;
            }
            ...
            cerr << "parseParameterRampWithVelocity() warning: null attributes in "
                    "Ramp tag\n";
...
vector<timedParameter> MidiNote::parseMultiParameterRamp(TiXmlElement* element) {
    TiXmlElement* ramp = element->FirstChildElement("Ramp");
    vector<timedParameter> out; // The output is a vector of deque objects.  Each
                                // deque holds a list of values to ramp to
    ...
    while (ramp != NULL) // These tags hold ramp values for time-variant parameters
    {
        ...
        if (rampVal != NULL && rampDur != NULL && rampType != NULL) {
            ...
            if (first) // Need to make the timedParameter vector big enough to hold
                       // all params
            ...
            if (rampType->compare(0, 3, "lin") == 0) // duration attribute, a single type attribute, and a list of
                                                     // values.
            {
#ifdef DEBUG_MESSAGES
                cout << "Linear ramp type\n";
            ...
            if (gated != NULL) {
                if (*gated == "true")
                    gateBool = true;
                cerr << "parseMultiParameterRamp(): gated = " << gateBool << endl;
            }

            size = (out.size() < values.size() ? out.size()
                                               : values.size()); // Minimum of size of output vector, value vector

            for (size_t i = 0; i < size; i++) // Go through each value in the list and add it to the
                                              // appropriate timedParameter
                out[i].push_back((parameterValue) { p, values[i], shape, gateBool });
        } else
            cerr << "assignPllSynthParameters() warning: null attributes in Ramp "
                    "tag\n";
...
// Parse a list of <Ramp> tags, each of whose values are a comma separated list,
// where each element of the list is of the form "a/b". The outputs are a pair
// of vectors of timedParameters.  Each timedParameter represents a series of
// ramp values for one parameter, so the vector holds multiple parameters, e.g.
// multiple harmonic amplitudes.  The trick is keeping the indexing straight.

void MidiNote::parseMultiParameterRampWithVelocity(TiXmlElement* element, vector<timedParameter>* out1,
                                                   vector<timedParameter>* out2) {
    TiXmlElement* ramp = element->FirstChildElement("Ramp");
    timedParameter emptyTp; // timedParameter with no parameterValue elements
    bool first = true;
    ...
    while (ramp != NULL) // These tags hold ramp values for time-variant parameters
    {
        ...
        if (rampVal != NULL && rampDur != NULL && rampType != NULL) {
            ...
            if (gated != NULL) {
                if (*gated == "true")
                    gateBool = true;
                cerr << "parseMultiParameterRampWithVelocity(): gated = " << gateBool << endl;
            }

            size_t size = val1.size(); // Minimum of size of output vectors, value vector
            if (out1 != NULL)
                if (out1->size() < size)
                    size = out1->size();
            if (out2 != NULL)
            ...
            {
                if (out1 != NULL)
                    (*out1)[i].push_back((parameterValue) { dur1, val1[i], shape, gateBool });
                if (out2 != NULL)
                    (*out2)[i].push_back((parameterValue) { dur2, val2[i], shape, gateBool });
            }
...
vector<double> MidiNote::parseCommaSeparatedValues(const string& inString) {
    ...
    // Parse a string of the format "0, 1.3, -1.23, 4" containing a list of double
    // values, and put the values into a vector Strings of the form "1,,,,,,3,4"
    // are acceptable-- empty values default to 0
...
// Parse a string of the format "1/2,2.5/2.7,0/100" storing the results in two
// vectors
...
// Parses strings of the format "a/b" indicating a range of velocity-sensitive
// values. Stores the two values in outA and outB; if the string only contains
// one number, outA = outB. Returns the number of values found (0, 1, 2)
...
// This extends the usual strtod method to allow dB values (possibly other
// suffixes in the future)
...
double MidiNote::SynthBaseFactory::transeg(double val1, double val2, double concavity, double velocity) {
    // Given a low and high value, a concavity and a velocity, return a normalized
    // value This operates similar to csound's transeg opcode.  concavity > 0
    // produces a slowly rising (concave) curve where concavity < 0 produces a
    // convex curve.  concavity = 0 produces a linear curve.
...
#ifdef DEBUG_MESSAGES_EXTRA
    cout << "transeg: input (" << val1 << ", " << val2 << ") conc. " << concavity << " vel. " << velocity << " output "
         << out << endl;
#endif
    return out;
}

PllSynth* MidiNote::PllSynthFactory::createSynth(int note, int velocity, float velocityCurvature,
    ...
    if (globalAmplitudeActive_) {
        start = amplitudeOffset
            * transeg(globalAmplitudeMin_.start, globalAmplitudeMax_.start, globalAmplitudeConcavity_, curvedVelocity);
        for (i = 0; i < min(globalAmplitudeMin_.ramp.size(), globalAmplitudeMax_.ramp.size()); i++) {
            pval.nextValue = amplitudeOffset
                * transeg(globalAmplitudeMin_.ramp[i].nextValue, globalAmplitudeMax_.ramp[i].nextValue,
            ...
            pval.gated = globalAmplitudeMin_.ramp[i].gated;
            ramp.push_back(pval);
        }
    ...
    if (loopGainActive_) {
        ...
        for (i = 0; i < min(loopGainMin_.ramp.size(), loopGainMax_.ramp.size()); i++) {
            pval.nextValue = transeg(loopGainMin_.ramp[i].nextValue, loopGainMax_.ramp[i].nextValue, loopGainConcavity_,
                                     curvedVelocity);
            pval.duration = transeg(loopGainMin_.ramp[i].duration, loopGainMax_.ramp[i].duration, loopGainConcavity_,
                                    curvedVelocity);
            pval.shape = loopGainMin_.ramp[i].shape;
            pval.gated = loopGainMin_.ramp[i].gated;
            ramp.push_back(pval);
        }
    ...
    if (amplitudeFeedbackScalerActive_) {
        start = transeg(amplitudeFeedbackScalerMin_.start, amplitudeFeedbackScalerMax_.start,
        ...
        for (i = 0; i < min(amplitudeFeedbackScalerMin_.ramp.size(), amplitudeFeedbackScalerMax_.ramp.size()); i++) {
            pval.nextValue =
                transeg(amplitudeFeedbackScalerMin_.ramp[i].nextValue, amplitudeFeedbackScalerMax_.ramp[i].nextValue,
                        amplitudeFeedbackScalerConcavity_, curvedVelocity);
            pval.duration =
                transeg(amplitudeFeedbackScalerMin_.ramp[i].duration, amplitudeFeedbackScalerMax_.ramp[i].duration,
                        amplitudeFeedbackScalerConcavity_, curvedVelocity);
            pval.shape = amplitudeFeedbackScalerMin_.ramp[i].shape;
            pval.gated = amplitudeFeedbackScalerMin_.ramp[i].gated;
            ramp.push_back(pval);
        }
    ...
    if (inputGainsActive_) {
        for (j = 0; j < min(inputGainsMin_.size(), inputGainsMax_.size()); j++) {
            ...
            for (i = 0; i < min(inputGainsMin_[j].ramp.size(), inputGainsMax_[j].ramp.size()); i++) {
                ...
                pval.gated = inputGainsMin_[j].ramp[i].gated;
                ramp.push_back(pval);
            }
            vtp.push_back(ramp);
            ramp.clear();
    ...
    if (inputDelaysActive_) {
        for (j = 0; j < min(inputDelaysMin_.size(), inputDelaysMax_.size()); j++) {
            ...
            for (i = 0; i < min(inputDelaysMin_[j].ramp.size(), inputDelaysMax_[j].ramp.size()); i++) {
                ...
                pval.gated = inputDelaysMin_[j].ramp[i].gated;
                ramp.push_back(pval);
            }
            vtp.push_back(ramp);
            ramp.clear();
    ...
    if (harmonicAmplitudesActive_) {
        for (j = 0; j < min(harmonicAmplitudesMin_.size(), harmonicAmplitudesMax_.size()); j++) {
            start = transeg(harmonicAmplitudesMin_[j].start, harmonicAmplitudesMax_[j].start,
            ...
            for (i = 0; i < min(harmonicAmplitudesMin_[j].ramp.size(), harmonicAmplitudesMax_[j].ramp.size()); i++) {
                pval.nextValue =
                    transeg(harmonicAmplitudesMin_[j].ramp[i].nextValue, harmonicAmplitudesMax_[j].ramp[i].nextValue,
                            harmonicAmplitudesConcavity_, curvedVelocity);
                pval.duration =
                    transeg(harmonicAmplitudesMin_[j].ramp[i].duration, harmonicAmplitudesMax_[j].ramp[i].duration,
                            harmonicAmplitudesConcavity_, curvedVelocity);
                pval.shape = harmonicAmplitudesMin_[j].ramp[i].shape;
                pval.gated = harmonicAmplitudesMin_[j].ramp[i].gated;
                ramp.push_back(pval);
            }
            vtp.push_back(ramp);
            ramp.clear();
    ...
    if (harmonicPhasesActive_) {
        for (j = 0; j < min(harmonicPhasesMin_.size(), harmonicPhasesMax_.size()); j++) {
            start = transeg(harmonicPhasesMin_[j].start, harmonicPhasesMax_[j].start, harmonicPhasesConcavity_,
                            curvedVelocity);
            vd.push_back(start);
            for (i = 0; i < min(harmonicPhasesMin_[j].ramp.size(), harmonicPhasesMax_[j].ramp.size()); i++) {
                pval.nextValue =
                    transeg(harmonicPhasesMin_[j].ramp[i].nextValue, harmonicPhasesMax_[j].ramp[i].nextValue,
                ...
                pval.gated = harmonicPhasesMin_[j].ramp[i].gated;
                ramp.push_back(pval);
            }
            vtp.push_back(ramp);
            ramp.clear();
    ...
    if (relativeFrequencyActive_) {
        start = baseFreq
            * transeg(relativeFrequencyMin_.start, relativeFrequencyMax_.start, relativeFrequencyConcavity_,
                      curvedVelocity);
        for (i = 0; i < min(relativeFrequencyMin_.ramp.size(), relativeFrequencyMax_.ramp.size()); i++) {
            pval.nextValue = baseFreq
                * transeg(relativeFrequencyMin_.ramp[i].nextValue, relativeFrequencyMax_.ramp[i].nextValue,
            ...
            pval.gated = relativeFrequencyMin_.ramp[i].gated;
            ramp.push_back(pval);
        }
...
NoiseSynth* MidiNote::NoiseSynthFactory::createSynth(int note, int velocity, float velocityCurvature,
                                                     float amplitudeOffset) {
    ...
    if (filterFrequenciesActive_) // Normal filter frequencies to base frequency
                                  // of the note
    {
        for (j = 0; j < min(filterFrequenciesMin_.size(), filterFrequenciesMax_.size()); j++) {
            start = baseFreq
                * transeg(filterFrequenciesMin_[j].start, filterFrequenciesMax_[j].start, filterFrequenciesConcavity_,
                          curvedVelocity);
            vd.push_back(start);
            for (i = 0; i < min(filterFrequenciesMin_[j].ramp.size(), filterFrequenciesMax_[j].ramp.size()); i++) {
                pval.nextValue = baseFreq
                    * transeg(filterFrequenciesMin_[j].ramp[i].nextValue, filterFrequenciesMax_[j].ramp[i].nextValue,
                              filterFrequenciesConcavity_, curvedVelocity);
                pval.duration =
                    transeg(filterFrequenciesMin_[j].ramp[i].duration, filterFrequenciesMax_[j].ramp[i].duration,
                            filterFrequenciesConcavity_, curvedVelocity);
                pval.shape = filterFrequenciesMin_[j].ramp[i].shape;
                pval.gated = filterFrequenciesMin_[j].ramp[i].gated;
                ramp.push_back(pval);
            }
            vtp.push_back(ramp);
            ramp.clear();
    ...
    if (filterQsActive_) {
        for (j = 0; j < min(filterQsMin_.size(), filterQsMax_.size()); j++) {
            ...
            for (i = 0; i < min(filterQsMin_[j].ramp.size(), filterQsMax_[j].ramp.size()); i++) {
                ...
                pval.gated = filterQsMin_[j].ramp[i].gated;
                ramp.push_back(pval);
            }
            vtp.push_back(ramp);
            ramp.clear();
    ...
    if (filterAmplitudesActive_) {
        for (j = 0; j < min(filterAmplitudesMin_.size(), filterAmplitudesMax_.size()); j++) {
            start = transeg(filterAmplitudesMin_[j].start, filterAmplitudesMax_[j].start, filterAmplitudesConcavity_,
                            curvedVelocity);
            vd.push_back(start);
            for (i = 0; i < min(filterAmplitudesMin_[j].ramp.size(), filterAmplitudesMax_[j].ramp.size()); i++) {
                pval.nextValue =
                    transeg(filterAmplitudesMin_[j].ramp[i].nextValue, filterAmplitudesMax_[j].ramp[i].nextValue,
                            filterAmplitudesConcavity_, curvedVelocity);
                pval.duration =
                    transeg(filterAmplitudesMin_[j].ramp[i].duration, filterAmplitudesMax_[j].ramp[i].duration,
                            filterAmplitudesConcavity_, curvedVelocity);
                pval.shape = filterAmplitudesMin_[j].ramp[i].shape;
                pval.gated = filterAmplitudesMin_[j].ramp[i].gated;
                ramp.push_back(pval);
            }
            vtp.push_back(ramp);
            ramp.clear();
...
CalibratorNote::CalibratorNote(MidiController* controller, AudioRender* render): MidiNote(controller, render) {
    ...
    // Set some important default values.  Actual frequency will be set at note-on
    // time; phase offset and global amplitude will be adjusted by controls
...
int CalibratorNote::parseXml(TiXmlElement* baseElement) {
    ...
    // The three controls above reflect phase offset, amplitude offset, and a
    // global calibration amplitude which doesn't get saved (only used to find a
    // comfortable volume for calibrating). There will always be one synth within
    // this note.  Its parameters are not controllable.
...
void CalibratorNote::midiControlChange(unsigned char channel, unsigned char control, unsigned char value) {
    ...
    // Listen for special controls that adjust phase offset, amplitude offset, and
    // save values Call up to MidiNote::midiControlChange to handle the damper and
    // sostenuto pedal issues

    if ((unsigned int)control == phaseControl_ && (unsigned int)channel == phaseControlChannel_) {
        // When the control for phaseOffset changes, save value and send a
        // setPhaseOffset message to synth Control range maps to a phase offset of
        // 0-1
        ...
        // When the control for amplitudeAdjust changes, save value and send a
        // setGlobalAmplitude message to synth Control range maps to amplitude
        // offset of -12dB to 6dB (0.25 to 2.0), with the lower half of the range
        // having more response, to attenuate particularly loud notes
...
// Set the phase offset for this particular note.  Called either from
// midiControlChange or by OSC.
...
// Set the amplitude offset for this particular note.  Called either from
// midiControlChange or by OSC.
...
CalibratorNote* CalibratorNote::createNote(int audioChannel, int mrpChannel, int midiNote, int midiChannel,
                                           int pianoString, unsigned int key, int priority, int velocity,
                                           float phaseOffset, float amplitudeOffset) {
    // Essentially duplicate this note.  This means the synth in the prototype
    // note is redundant, but perhaps later we'll want to control its parameters
    // from the XML file, so we won't worry about the extra memory too much.
    ...
    // Tell each synth to update its center frequency; there should only be one
    // synth but who knows....
    for (i = 0; i < synths_.size(); i++) {
        if (get_ptr_typeid(synths_[i]) != typeid(PllSynth))
            continue;
...
// This method registers for specific OSC paths when the OscController object is
// set.  We need a reference to the controller so we can unregister on
// destruction.  Unregistering is handled by the OscHandler destructor.
...
// registered for.  Use it to change calibration values.  Returns true if
// message was handled.
...
// Parse an XML data structure to get all the parameters we need.  This is used
// to get data for the ResonanceSynth object
...
int ResonanceNote::parseXml(TiXmlElement* baseElement) {
    ...
    while (synthElement != NULL) {
        const string* synthClass =
            synthElement->Attribute((const string) "class"); // Attribute tells us the kind of synth
...
// This gets called whenever a new MIDI note comes in.  We care only about note
// on messages, not note off. If it's one of the harmonics of this note, send it
// on to the synthesizer.
...
void ResonanceNote::midiNoteEvent(unsigned char status, unsigned char note, unsigned char velocity) {
    ...
    // See if the incoming note is harmonically related to the pitch of the active
    // note
    if (harmonicallyRelated_.count(note - midiNote_) > 0) {
        if (synths_.size() > 0) {
            ...
            if (get_ptr_typeid(synths_[0]) == typeid(ResonanceSynth))
                ((ResonanceSynth*)synths_[0])->addHarmonic(note, frequency, amplitude);
        }
    }
}

ResonanceNote* ResonanceNote::createNote(int audioChannel, int mrpChannel, int midiNote, int midiChannel,
                                         int pianoString, unsigned int key, int priority, int velocity,
                                         float phaseOffset, float amplitudeOffset) {
    ...
    for (i = 0; i < synths_.size(); i++) {
        if (get_ptr_typeid(synths_[i]) != typeid(ResonanceSynth))
            continue;
