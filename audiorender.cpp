...
AudioRender::AudioRender(): pollCallback_(nullptr), pollCallbackArg_(nullptr) {
    // Initialize a mutex which prevents changing the render list while it's
    // rendering
    ...
    // Similar mutex prevents problems with multi-threaded requests for channel
    // allocation
    ...
    numStreamOutputChannels_ = 0;
    globalAmplitude_ = 1.0;
}

// Set the basic stream information

void AudioRender::setStreamInfo(PaStream* stream, int numInputChannels, int numOutputChannels,
                                int numStreamOutputChannels, float sampleRate, const vector<int>& channelsToUse,
                                const vector<int>& downmixChannels) {
    int i;

    stream_ = stream;
    numInputChannels_ = numInputChannels;
    numOutputChannels_ = numOutputChannels;
    numStreamOutputChannels_ = numStreamOutputChannels;
    sampleRate_ = sampleRate;
    downmixChannels_ = downmixChannels;
    if (numOutputChannels_ != numStreamOutputChannels_) {
        // this is just an educated guess. Reallocation may happen upon first call to renderCallback()
        outputBuffer_.resize(numOutputChannels_ * 64);
    }

    // Set up the output channel tracking vector, telling us whether each channel
    // is in use
    channelsUsed_.resize(numOutputChannels, false);
    // Right now we don't change the state of any existing channels.  If we
    // eventually want to run setStreamInfo multiple times, consider whether this
...
// Set a list of channels which should be skipped on the MRP controller.
// Useful in case of connection faults.
void AudioRender::setMRPControllerChannelsToSkip(vector<int>& channelsToSkip) {
    mrpControllerSkipChannels_ = channelsToSkip;
}

// Returns two variables, the first is the index of the audio channel in the
// render buffer The second is the index of which MRP channel to use.  These are
// often the same, but in the case where we only want to use a subset of a
// device's channels, the MRP index might be different. Returns <-1,-1> on
// error.
...
pair<int, int> AudioRender::allocateOutputChannel() {
    ...
    for (channelIndex = 0; channelIndex < outputChannels_.size(); channelIndex++) {
        ...
        // Check this channel is on the list of MRP channels to skip
        for (int usableChannelIndex = 0; usableChannelIndex < mrpControllerSkipChannels_.size(); usableChannelIndex++) {
            if (channelIndex == mrpControllerSkipChannels_[usableChannelIndex]) {
    ...
    if (downmixChannels_.size()) {
        out.first = downmixChannels_[channelIndex % downmixChannels_.size()];
    }
...
int AudioRender::removeSynth(SynthBase* synth) {
    ...
    size_t ret = synths_.erase(synth);
...
// registered for.  In this case it's for changing the global volume.  Returns
// true on success.
...
int AudioRender::renderCallback(const void* input, void* output, unsigned long frameCount,
                                const PaStreamCallbackTimeInfo* timeInfo, PaStreamCallbackFlags statusFlags) {
    if (pollCallback_) {
        pollCallback_(pollCallbackArg_);
    }

    // First, initialize the output to all zeros
    bzero(output, frameCount * numStreamOutputChannels_ * sizeof(float));

    // Walk through the list of synths, calling the render process for each, which
    // will mix its output into the buffer (i.e. not overwrite what's already
    // there). Each synth already knows the sample rate and channel count.
    ...
    if (numOutputChannels_ == numStreamOutputChannels_) {
        ...
        outputBuffer_.resize(frameCount * numOutputChannels_); // may reallocate the first time
        bzero(outputBuffer_.data(), frameCount * numOutputChannels_ * sizeof(float));
        synthOutput = outputBuffer_.data();
    }
    set<SynthBase*>::iterator it;
    for (it = synths_.begin(); it != synths_.end(); it++) {
        (*it)->render(input, synthOutput, frameCount, timeInfo, statusFlags);
    }

    // Scale the output if necessary
    if (globalAmplitude_ != 1.0) {
        for (int i = 0; i < frameCount * numStreamOutputChannels_; i++)
            ((float*)output)[i] *= globalAmplitude_;
    }

    if (numOutputChannels_ != numStreamOutputChannels_) {
        // copy buffer to output, downmixing if appropriate
        for (unsigned int c = 0; c < numOutputChannels_; ++c) {
            unsigned int oc;
            if (downmixChannels_.size())
                oc = downmixChannels_[c % downmixChannels_.size()];
            else {
                if (c < numStreamOutputChannels_)
                    oc = c;
                else
                    continue; // discard extra channels
            }
            for (unsigned int n = 0; n < frameCount; ++n) {
                ((float*)output)[n * numStreamOutputChannels_ + oc] +=
                    ((float*)synthOutput)[n * numOutputChannels_ + c];
            }
        }
    }
...
void AudioRender::setPollCallback(void (*pollCallback)(void*), void* arg) {
    if (!pollCallback) {
        arg = nullptr;
    }
    pollCallbackArg_ = arg;
    pollCallback_ = pollCallback;
}

AudioRender::~AudioRender() { pthread_mutex_destroy(&renderMutex_); }
